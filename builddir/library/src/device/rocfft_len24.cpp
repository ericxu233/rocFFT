 ...
 * Kernel is: namespace(factors=[8, 3], flavour='uwide', length=24, scheme='CS_KERNEL_STOCKHAM', threads_per_block=256)
 */
...
__device__ void ip_forward_length24_SBRR_device(scalar_type* __restrict__ lds,
...
__global__
    __launch_bounds__(256) void ip_forward_length24_SBRR(const scalar_type* __restrict__ twiddles,
                                                         const size_t dim,
                                                         ...
                                                         scalar_type* __restrict__ buf)
{
    ...
    offset_lds = (24 + lds_padding) * (transform % 32); /* stockham.py:373 */
    ...
    if(ebtype == EmbeddedType::C2Real_PRE)
    {
        ...
        real_pre_process_kernel_inplace<scalar_type, true>(thread % 12 + 0,
                                                           24 - thread % 12 - 0,
                                                           12,
                                                           &lds[offset_lds],
                                                           0,
                                                           &twiddles[24]); /* stockham.py:338 */

        real_pre_process_kernel_inplace<scalar_type, true>(thread % 12 + 8,
                                                           24 - thread % 12 - 8,
                                                           12,
                                                           &lds[offset_lds],
                                                           0,
    ...
    ip_forward_length24_SBRR_device<scalar_type, SB_UNIT>(
    ...
    if(ebtype == EmbeddedType::Real2C_POST)
    {
        ...
        real_post_process_kernel_inplace<scalar_type, true>(thread % 12 + 0,
                                                            24 - thread % 12 - 0,
                                                            12,
                                                            &lds[offset_lds],
                                                            0,
        ...
        real_post_process_kernel_inplace<scalar_type, true>(thread % 12 + 8,
                                                            24 - thread % 12 - 8,
                                                            12,
                                                            &lds[offset_lds],
                                                            0,
...
/* stockham.py:709 */
template <typename scalar_type, StrideBin sb, EmbeddedType ebtype, CallbackType cbtype>
__global__ __launch_bounds__(256) void ip_forward_length24_SBRR(
    const scalar_type* __restrict__ twiddles,
    const size_t dim,
    const size_t* __restrict__ lengths,
    const size_t* __restrict__ stride,
...
/* stockham.py:765 */
template <typename scalar_type, StrideBin sb>
__device__ void op_forward_length24_SBRR_device(scalar_type* __restrict__ lds,
                                                const scalar_type* __restrict__ twiddles,
                                                size_t       stride_lds,
                                                unsigned int offset_lds,
                                                bool         write)
...
template <typename scalar_type, StrideBin sb, EmbeddedType ebtype, CallbackType cbtype>
__global__
    __launch_bounds__(256) void op_forward_length24_SBRR(const scalar_type* __restrict__ twiddles,
                                                         const size_t dim,
                                                         const size_t* __restrict__ lengths,
                                                         ...
                                                         scalar_type* __restrict__ buf_out)
{
    ...
    // transform
    write = true; /* stockham.py:681 */
    op_forward_length24_SBRR_device<scalar_type, SB_UNIT>(
        lds, twiddles, stride_lds, offset_lds, write); /* stockham.py:686 */
...
/* stockham.py:709 */
template <typename scalar_type, StrideBin sb, EmbeddedType ebtype, CallbackType cbtype>
__global__ __launch_bounds__(256) void op_forward_length24_SBRR(
    const scalar_type* __restrict__ twiddles,
    const size_t dim,
    const size_t* __restrict__ lengths,
    const size_t* __restrict__ stride_in,
...
/* stockham.py:765 */
template <typename scalar_type, StrideBin sb>
__device__ void ip_inverse_length24_SBRR_device(scalar_type* __restrict__ lds,
                                                const scalar_type* __restrict__ twiddles,
                                                size_t       stride_lds,
                                                unsigned int offset_lds,
                                                bool         write)
...
template <typename scalar_type, StrideBin sb, EmbeddedType ebtype, CallbackType cbtype>
__global__
    __launch_bounds__(256) void ip_inverse_length24_SBRR(const scalar_type* __restrict__ twiddles,
                                                         const size_t dim,
                                                         const size_t* __restrict__ lengths,
                                                         const size_t* __restrict__ stride,
                                                         const size_t       nbatch,
                                                         ...
                                                         scalar_type* __restrict__ buf)
{
    ...
    // transform
    write = true; /* stockham.py:681 */
    ip_inverse_length24_SBRR_device<scalar_type, SB_UNIT>(
        lds, twiddles, stride_lds, offset_lds, write); /* stockham.py:686 */
...
/* stockham.py:709 */
template <typename scalar_type, StrideBin sb, EmbeddedType ebtype, CallbackType cbtype>
__global__ __launch_bounds__(256) void ip_inverse_length24_SBRR(
    const scalar_type* __restrict__ twiddles,
    const size_t dim,
    const size_t* __restrict__ lengths,
    const size_t* __restrict__ stride,
...
/* stockham.py:765 */
template <typename scalar_type, StrideBin sb>
__device__ void op_inverse_length24_SBRR_device(scalar_type* __restrict__ lds,
                                                const scalar_type* __restrict__ twiddles,
                                                size_t       stride_lds,
                                                unsigned int offset_lds,
                                                bool         write)
...
template <typename scalar_type, StrideBin sb, EmbeddedType ebtype, CallbackType cbtype>
__global__
    __launch_bounds__(256) void op_inverse_length24_SBRR(const scalar_type* __restrict__ twiddles,
                                                         const size_t dim,
                                                         const size_t* __restrict__ lengths,
                                                         const size_t* __restrict__ stride_in,
                                                         const size_t* __restrict__ stride_out,
                                                         ...
                                                         scalar_type* __restrict__ buf_out)
{
    ...
    // transform
    write = true; /* stockham.py:681 */
    op_inverse_length24_SBRR_device<scalar_type, SB_UNIT>(
        lds, twiddles, stride_lds, offset_lds, write); /* stockham.py:686 */
...
/* stockham.py:709 */
template <typename scalar_type, StrideBin sb, EmbeddedType ebtype, CallbackType cbtype>
__global__ __launch_bounds__(256) void op_inverse_length24_SBRR(
    const scalar_type* __restrict__ twiddles,
    const size_t dim,
    const size_t* __restrict__ lengths,
    const size_t* __restrict__ stride_in,
...
POWX_SMALL_GENERATOR(rocfft_internal_dfn_dp_ci_ci_stoc_24,
                     ip_forward_length24_SBRR,
                     ip_inverse_length24_SBRR,
                     op_forward_length24_SBRR,
                     op_inverse_length24_SBRR,
                     double2);
POWX_SMALL_GENERATOR(rocfft_internal_dfn_sp_ci_ci_stoc_24,
                     ip_forward_length24_SBRR,
                     ip_inverse_length24_SBRR,
                     op_forward_length24_SBRR,
                     op_inverse_length24_SBRR,
