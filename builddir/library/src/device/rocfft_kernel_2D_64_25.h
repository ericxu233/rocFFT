...
fft_fwd_ip_2D_64_25( const T * __restrict__ twiddles, const size_t dim, const size_t *_lengths, const size_t *_stride_in, const size_t batch_count, const unsigned int lds_padding, void* __restrict__ load_cb_fn, void* __restrict__ load_cb_data, uint32_t load_cb_lds_bytes, void* __restrict__ store_cb_fn, void* __restrict__ store_cb_data, T * __restrict__ _gb)
{
	...
	fwd_len64_device<T, sb, cbtype>(twiddles, stride_in[0], stride_out[0],  rw, b, me%16, (me/16)*64, gbIn, iOffset, gbOut, oOffset, lds, load_cb_fn, load_cb_data, 0, nullptr, nullptr);
	}
	...
	// Perform FFT input: gb(In) ; output: gb(Out); working space: lds 
	// rw, b, me% control read/write; then ldsOffset, gb, lds
	fwd_len25_device<T, SB_NONUNIT, cbtype>(twiddles, stride_in[0], stride_out[0],  rw, b, me%5, (me/5)*25, gbIn, iOffset, gbOut, oOffset, lds, nullptr, nullptr, load_cb_lds_bytes, store_cb_fn, store_cb_data);
	}
}
...
__global__ void
__launch_bounds__(400)
fft_back_ip_2D_64_25( const T * __restrict__ twiddles, const size_t dim, const size_t *_lengths, const size_t *_stride_in, const size_t batch_count, const unsigned int lds_padding, void* __restrict__ load_cb_fn, void* __restrict__ load_cb_data, uint32_t load_cb_lds_bytes, void* __restrict__ store_cb_fn, void* __restrict__ store_cb_data, T * __restrict__ _gb)
{
	...
	// Perform FFT input: gb(In) ; output: gb(Out); working space: lds 
	// rw, b, me% control read/write; then ldsOffset, gb, lds
	back_len64_device<T, sb, cbtype>(twiddles, stride_in[0], stride_out[0],  rw, b, me%16, (me/16)*64, gbIn, iOffset, gbOut, oOffset, lds, load_cb_fn, load_cb_data, 0, nullptr, nullptr);
	}
	// we have two twiddle tables back to back in device
	// memory - move to the second table (if nonsquare)
	twiddles = twiddles + lengths[0];
	...
	// Perform FFT input: gb(In) ; output: gb(Out); working space: lds 
	// rw, b, me% control read/write; then ldsOffset, gb, lds
	back_len25_device<T, SB_NONUNIT, cbtype>(twiddles, stride_in[0], stride_out[0],  rw, b, me%5, (me/5)*25, gbIn, iOffset, gbOut, oOffset, lds, nullptr, nullptr, load_cb_lds_bytes, store_cb_fn, store_cb_data);
	}
}
...
fft_fwd_ip_2D_64_25( const T * __restrict__ twiddles, const size_t dim, const size_t *_lengths, const size_t *_stride_in, const size_t batch_count, const unsigned int lds_padding, void* __restrict__ load_cb_fn, void* __restrict__ load_cb_data, uint32_t load_cb_lds_bytes, void* __restrict__ store_cb_fn, void* __restrict__ store_cb_data, real_type_t<T> * __restrict__ _gbRe, real_type_t<T> * __restrict__ _gbIm)
{
	...
	// Perform FFT input: gb(In) ; output: gb(Out); working space: lds 
	// rw, b, me% control read/write; then ldsOffset, gb, lds
	fwd_len64_device<T, sb, cbtype>(twiddles, stride_in[0], stride_out[0],  rw, b, me%16, (me/16)*64, gbInRe, gbInIm, iOffset, gbOut, oOffset, lds, load_cb_fn, load_cb_data, 0, nullptr, nullptr);
	}
	// we have two twiddle tables back to back in device
	// memory - move to the second table (if nonsquare)
	twiddles = twiddles + lengths[0];
	...
	// Perform FFT input: gb(In) ; output: gb(Out); working space: lds 
	// rw, b, me% control read/write; then ldsOffset, gb, lds
	fwd_len25_device<T, SB_NONUNIT, cbtype>(twiddles, stride_in[0], stride_out[0],  rw, b, me%5, (me/5)*25, gbIn, iOffset, gbOutRe, gbOutIm, oOffset, lds, nullptr, nullptr, load_cb_lds_bytes, store_cb_fn, store_cb_data);
	}
}
...
fft_back_ip_2D_64_25( const T * __restrict__ twiddles, const size_t dim, const size_t *_lengths, const size_t *_stride_in, const size_t batch_count, const unsigned int lds_padding, void* __restrict__ load_cb_fn, void* __restrict__ load_cb_data, uint32_t load_cb_lds_bytes, void* __restrict__ store_cb_fn, void* __restrict__ store_cb_data, real_type_t<T> * __restrict__ _gbRe, real_type_t<T> * __restrict__ _gbIm)
{
	...
	// Perform FFT input: gb(In) ; output: gb(Out); working space: lds 
	// rw, b, me% control read/write; then ldsOffset, gb, lds
	back_len64_device<T, sb, cbtype>(twiddles, stride_in[0], stride_out[0],  rw, b, me%16, (me/16)*64, gbInRe, gbInIm, iOffset, gbOut, oOffset, lds, load_cb_fn, load_cb_data, 0, nullptr, nullptr);
	}
	// we have two twiddle tables back to back in device
	// memory - move to the second table (if nonsquare)
	twiddles = twiddles + lengths[0];
	...
	// Perform FFT input: gb(In) ; output: gb(Out); working space: lds 
	// rw, b, me% control read/write; then ldsOffset, gb, lds
	back_len25_device<T, SB_NONUNIT, cbtype>(twiddles, stride_in[0], stride_out[0],  rw, b, me%5, (me/5)*25, gbIn, iOffset, gbOutRe, gbOutIm, oOffset, lds, nullptr, nullptr, load_cb_lds_bytes, store_cb_fn, store_cb_data);
	}
}
...
__global__ void
__launch_bounds__(400)
fft_back_op_2D_64_25( const T * __restrict__ twiddles, const size_t dim, const size_t *_lengths, const size_t *_stride_in, const size_t *_stride_out, const size_t batch_count, const unsigned int lds_padding, void* __restrict__ load_cb_fn, void* __restrict__ load_cb_data, uint32_t load_cb_lds_bytes, void* __restrict__ store_cb_fn, void* __restrict__ store_cb_data, T * __restrict__ _gbIn, T * __restrict__ _gbOut)
{
