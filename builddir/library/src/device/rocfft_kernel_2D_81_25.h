...
fft_fwd_ip_2D_81_25( cl::sycl::range<3> blocks, cl::sycl::range<3> threads, cl::sycl::queue rocfftQueue, cl::sycl::buffer<T, 1> twiddles_GB, const size_t dim, cl::sycl::buffer<size_t, 1> *_lengths_GB, cl::sycl::buffer<size_t, 1> *_stride_in_GB, const size_t batch_count, cl::sycl::buffer<T, 1> _gb_GB)
{
rocfftQueue.submit([&](cl::sycl::handler &cgh)
{
	...
	cgh.parallel_for<class kern_fft_fwd_ip_2D_81_25>(
...
//// Print kernel code here (after function prototype)
	...
	size_t counter_mod = (batch*0 + (me/5));
	if(dim == 1){
		iOffset += counter_mod*stride_in[1];
		oOffset += counter_mod*stride_out[1];
	}
	...
	// Perform FFT input: lwb(In) ; output: lwb(Out); working space: lds 
	// rw, b, me% control read/write; then ldsOffset, lwb, lds
	fwd_len25_device<T, SB_NONUNIT>(twiddles, stride_in[0], stride_out[0],  rw, b, me%5, (me/5)*25, lwbIn, lwbOut, lds);
	}
});
});
}
...
__global__ void
__launch_bounds__(256)
fft_back_ip_2D_81_25( cl::sycl::range<3> blocks, cl::sycl::range<3> threads, cl::sycl::queue rocfftQueue, cl::sycl::buffer<T, 1> twiddles_GB, const size_t dim, cl::sycl::buffer<size_t, 1> *_lengths_GB, cl::sycl::buffer<size_t, 1> *_stride_in_GB, const size_t batch_count, cl::sycl::buffer<T, 1> _gb_GB)
{
rocfftQueue.submit([&](cl::sycl::handler &cgh)
{
	auto twiddles = twiddles_GB.get_access<cl::sycl::access::mode::read>(cgh);
	...
	auto _gb = _gb_GB.get_access<cl::sycl::access::mode::read_write>(cgh);
	cl::sycl::accessor<real_type_t<T>,1,sycl::access::mode::read_write,sycl::access::target::local> lds(cl::sycl::range<1>(2025), cgh);
	cgh.parallel_for<class kern_fft_back_ip_2D_81_25>(
	                   cl::sycl::nd_range<3>(blocks, threads),
	                   [=](cl::sycl::nd_item<3> wItem)
{
//// Print kernel code here (after function prototype)
	...
	// Perform FFT input: lwb(In) ; output: lwb(Out); working space: lds 
	// rw, b, me% control read/write; then ldsOffset, lwb, lds
	back_len25_device<T, SB_NONUNIT>(twiddles, stride_in[0], stride_out[0],  rw, b, me%5, (me/5)*25, lwbIn, lwbOut, lds);
	}
});
});
}
...
//// Print kernel code here (after function prototype)
	...
	// Perform FFT input: lwb(In) ; output: lwb(Out); working space: lds 
	// rw, b, me% control read/write; then ldsOffset, lwb, lds
	fwd_len25_device<T, SB_NONUNIT>(twiddles, stride_in[0], stride_out[0],  rw, b, me%5, (me/5)*25, lwbIn, lwbOutRe, lwbOutIm, lds);
	}
});
});
}
...
__global__ void
__launch_bounds__(256)
fft_back_ip_2D_81_25( cl::sycl::range<3> blocks, cl::sycl::range<3> threads, cl::sycl::queue rocfftQueue, cl::sycl::buffer<T, 1> twiddles_GB, const size_t dim, cl::sycl::buffer<size_t, 1> *_lengths_GB, cl::sycl::buffer<size_t, 1> *_stride_in_GB, const size_t batch_count, cl::sycl::buffer<real_type_t<T>, 1> _gbRe_GB, cl::sycl::buffer<real_type_t<T>, 1> _gbIm_GB)
{
rocfftQueue.submit([&](cl::sycl::handler &cgh)
{
	auto twiddles = twiddles_GB.get_access<cl::sycl::access::mode::read>(cgh);
...
//// Print kernel code here (after function prototype)
	...
	// Perform FFT input: lwb(In) ; output: lwb(Out); working space: lds 
	// rw, b, me% control read/write; then ldsOffset, lwb, lds
	back_len25_device<T, SB_NONUNIT>(twiddles, stride_in[0], stride_out[0],  rw, b, me%5, (me/5)*25, lwbIn, lwbOutRe, lwbOutIm, lds);
	}
});
});
}
...
rocfftQueue.submit([&](cl::sycl::handler &cgh)
{
	...
	cl::sycl::accessor<real_type_t<T>,1,sycl::access::mode::read_write,sycl::access::target::local> lds(cl::sycl::range<1>(2025), cgh);
	cgh.parallel_for<class kern_fft_fwd_op_2D_81_25>(
	                   cl::sycl::nd_range<3>(blocks, threads),
	                   [=](cl::sycl::nd_item<3> wItem)
{
//// Print kernel code here (after function prototype)
...
rocfftQueue.submit([&](cl::sycl::handler &cgh)
{
	...
	auto _gbOut = _gbOut_GB.get_access<cl::sycl::access::mode::read_write>(cgh);
	cl::sycl::accessor<real_type_t<T>,1,sycl::access::mode::read_write,sycl::access::target::local> lds(cl::sycl::range<1>(2025), cgh);
	cgh.parallel_for<class kern_fft_back_op_2D_81_25>(
	                   cl::sycl::nd_range<3>(blocks, threads),
	                   [=](cl::sycl::nd_item<3> wItem)
{
//// Print kernel code here (after function prototype)
