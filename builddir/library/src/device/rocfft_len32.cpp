 ...
 * Kernel is: namespace(factors=[16, 2], flavour='uwide', length=32, scheme='CS_KERNEL_STOCKHAM', threads_per_block=64)
 */
                                                ...
                                                bool         write)
{
    ...
    R[11] = lds[offset_lds + (thread + 22) * lstride]; /* stockham.py:542 */
    R[12] = lds[offset_lds + (thread + 24) * lstride]; /* stockham.py:542 */
    R[13] = lds[offset_lds + (thread + 26) * lstride]; /* stockham.py:542 */
    R[14] = lds[offset_lds + (thread + 28) * lstride]; /* stockham.py:542 */
    R[15] = lds[offset_lds + (thread + 30) * lstride]; /* stockham.py:542 */
    ...
    W    = twiddles[15 + 1 * (thread % 16)]; /* stockham.py:554 */
    ...
    if(write)
    {
        lds[offset_lds + ((thread / 16) * 32 + thread % 16 + 0) * lstride]
            = R[0]; /* stockham.py:577 */
        lds[offset_lds + ((thread / 16) * 32 + thread % 16 + 16) * lstride]
            = R[1]; /* stockham.py:577 */
    }
}
/* stockham.py:709 */
...
__global__
    __launch_bounds__(64) void ip_forward_length32_SBRR(const scalar_type* __restrict__ twiddles,
                                                        const size_t dim,
                                                        ...
                                                        scalar_type* __restrict__ buf)
{
    ...
    offset_lds = (32 + lds_padding) * (transform % 4); /* stockham.py:373 */
    ...
    if(ebtype == EmbeddedType::C2Real_PRE)
    {
        ...
        real_pre_process_kernel_inplace<scalar_type, true>(thread % 16 + 0,
                                                           32 - thread % 16 - 0,
                                                           16,
                                                           &lds[offset_lds],
                                                           0,
                                                           &twiddles[32]); /* stockham.py:338 */
    ...
    if(ebtype == EmbeddedType::Real2C_POST)
    {
        ...
        real_post_process_kernel_inplace<scalar_type, true>(thread % 16 + 0,
                                                            32 - thread % 16 - 0,
                                                            16,
                                                            &lds[offset_lds],
                                                            0,
...
/* stockham.py:709 */
template <typename scalar_type, StrideBin sb, EmbeddedType ebtype, CallbackType cbtype>
__global__ __launch_bounds__(64) void ip_forward_length32_SBRR(
    const scalar_type* __restrict__ twiddles,
    const size_t dim,
    const size_t* __restrict__ lengths,
    const size_t* __restrict__ stride,
...
template <typename scalar_type, StrideBin sb, EmbeddedType ebtype, CallbackType cbtype>
__global__
    __launch_bounds__(64) void op_forward_length32_SBRR(const scalar_type* __restrict__ twiddles,
                                                        const size_t dim,
                                                        const size_t* __restrict__ lengths,
...
/* stockham.py:709 */
template <typename scalar_type, StrideBin sb, EmbeddedType ebtype, CallbackType cbtype>
__global__ __launch_bounds__(64) void op_forward_length32_SBRR(
    const scalar_type* __restrict__ twiddles,
    const size_t dim,
    const size_t* __restrict__ lengths,
    const size_t* __restrict__ stride_in,
                                                ...
                                                bool         write)
{
    ...
    W    = {twiddles[15 + 1 * (thread % 16)].x,
         -twiddles[15 + 1 * (thread % 16)].y}; /* generator.py:1065 */
    t.x  = W.x * R[1].x - W.y * R[1].y; /* stockham.py:555 */
    t.y  = W.y * R[1].x + W.x * R[1].y; /* stockham.py:556 */
    R[1] = t; /* stockham.py:557 */
...
template <typename scalar_type, StrideBin sb, EmbeddedType ebtype, CallbackType cbtype>
__global__
    __launch_bounds__(64) void ip_inverse_length32_SBRR(const scalar_type* __restrict__ twiddles,
                                                        const size_t dim,
                                                        const size_t* __restrict__ lengths,
                                                        const size_t* __restrict__ stride,
                                                        const size_t       nbatch,
...
/* stockham.py:709 */
template <typename scalar_type, StrideBin sb, EmbeddedType ebtype, CallbackType cbtype>
__global__ __launch_bounds__(64) void ip_inverse_length32_SBRR(
    const scalar_type* __restrict__ twiddles,
    const size_t dim,
    const size_t* __restrict__ lengths,
    const size_t* __restrict__ stride,
...
template <typename scalar_type, StrideBin sb, EmbeddedType ebtype, CallbackType cbtype>
__global__
    __launch_bounds__(64) void op_inverse_length32_SBRR(const scalar_type* __restrict__ twiddles,
                                                        const size_t dim,
                                                        const size_t* __restrict__ lengths,
                                                        const size_t* __restrict__ stride_in,
                                                        const size_t* __restrict__ stride_out,
...
/* stockham.py:709 */
template <typename scalar_type, StrideBin sb, EmbeddedType ebtype, CallbackType cbtype>
__global__ __launch_bounds__(64) void op_inverse_length32_SBRR(
    const scalar_type* __restrict__ twiddles,
    const size_t dim,
    const size_t* __restrict__ lengths,
    const size_t* __restrict__ stride_in,
...
POWX_SMALL_GENERATOR(rocfft_internal_dfn_dp_ci_ci_stoc_32,
                     ip_forward_length32_SBRR,
                     ip_inverse_length32_SBRR,
                     op_forward_length32_SBRR,
                     op_inverse_length32_SBRR,
                     double2);
POWX_SMALL_GENERATOR(rocfft_internal_dfn_sp_ci_ci_stoc_32,
                     ip_forward_length32_SBRR,
                     ip_inverse_length32_SBRR,
                     op_forward_length32_SBRR,
                     op_inverse_length32_SBRR,
