/*
 * Stockham kernels generated by:
 * 
 *     /home/ugrad/ugrad003/roctosycl/rocFFT/library/src/device/kernel-generator.py --pattern=all --precision=all --groups=150 --manual-small= --manual-large= generate /home/ugrad/ugrad003/roctosycl/rocFFT/builddir/library/src/device/generator/rocfft-kernel-generator
 * 
 * Generator is: /home/ugrad/ugrad003/roctosycl/rocFFT/library/src/device/kernel-generator.py
 * Kernel is: namespace(factors=[5, 5, 3], flavour='uwide', length=75, scheme='CS_KERNEL_STOCKHAM', threads_per_block=256)
 */

#include "kernel_launch.h"
#include "real2complex.h"
#include "rocfft_butterfly_template.h"
#include <hip/hip_runtime.h>

/* stockham.py:765 */
template <typename scalar_type, StrideBin sb>
__device__ void ip_forward_length75_SBRR_device(scalar_type* __restrict__ lds,
                                                const scalar_type* __restrict__ twiddles,
                                                size_t       stride_lds,
                                                unsigned int offset_lds,
                                                bool         write)
{
    size_t       thread;
    scalar_type  R[5];
    scalar_type  W;
    scalar_type  t;
    const size_t lstride = (sb == SB_UNIT) ? (1) : (stride_lds);
    thread               = threadIdx.x % 25; /* stockham.py:731 */

    // pass 0
    __syncthreads();
    R[0] = lds[offset_lds + (thread + 0) * lstride]; /* stockham.py:542 */
    R[1] = lds[offset_lds + (thread + 15) * lstride]; /* stockham.py:542 */
    R[2] = lds[offset_lds + (thread + 30) * lstride]; /* stockham.py:542 */
    R[3] = lds[offset_lds + (thread + 45) * lstride]; /* stockham.py:542 */
    R[4] = lds[offset_lds + (thread + 60) * lstride]; /* stockham.py:542 */

    FwdRad5B1(&R[0], &R[1], &R[2], &R[3], &R[4]); /* stockham.py:566 */

    __syncthreads();
    if(write && thread < 15)
    {
        lds[offset_lds + ((thread / 1) * 5 + thread % 1 + 0) * lstride]
            = R[0]; /* stockham.py:577 */
        lds[offset_lds + ((thread / 1) * 5 + thread % 1 + 1) * lstride]
            = R[1]; /* stockham.py:577 */
        lds[offset_lds + ((thread / 1) * 5 + thread % 1 + 2) * lstride]
            = R[2]; /* stockham.py:577 */
        lds[offset_lds + ((thread / 1) * 5 + thread % 1 + 3) * lstride]
            = R[3]; /* stockham.py:577 */
        lds[offset_lds + ((thread / 1) * 5 + thread % 1 + 4) * lstride]
            = R[4]; /* stockham.py:577 */
    }

    // pass 1
    __syncthreads();
    R[0] = lds[offset_lds + (thread + 0) * lstride]; /* stockham.py:542 */
    R[1] = lds[offset_lds + (thread + 15) * lstride]; /* stockham.py:542 */
    R[2] = lds[offset_lds + (thread + 30) * lstride]; /* stockham.py:542 */
    R[3] = lds[offset_lds + (thread + 45) * lstride]; /* stockham.py:542 */
    R[4] = lds[offset_lds + (thread + 60) * lstride]; /* stockham.py:542 */

    W    = twiddles[4 + 4 * (thread % 5)]; /* stockham.py:554 */
    t.x  = W.x * R[1].x - W.y * R[1].y; /* stockham.py:555 */
    t.y  = W.y * R[1].x + W.x * R[1].y; /* stockham.py:556 */
    R[1] = t; /* stockham.py:557 */
    W    = twiddles[5 + 4 * (thread % 5)]; /* stockham.py:554 */
    t.x  = W.x * R[2].x - W.y * R[2].y; /* stockham.py:555 */
    t.y  = W.y * R[2].x + W.x * R[2].y; /* stockham.py:556 */
    R[2] = t; /* stockham.py:557 */
    W    = twiddles[6 + 4 * (thread % 5)]; /* stockham.py:554 */
    t.x  = W.x * R[3].x - W.y * R[3].y; /* stockham.py:555 */
    t.y  = W.y * R[3].x + W.x * R[3].y; /* stockham.py:556 */
    R[3] = t; /* stockham.py:557 */
    W    = twiddles[7 + 4 * (thread % 5)]; /* stockham.py:554 */
    t.x  = W.x * R[4].x - W.y * R[4].y; /* stockham.py:555 */
    t.y  = W.y * R[4].x + W.x * R[4].y; /* stockham.py:556 */
    R[4] = t; /* stockham.py:557 */

    FwdRad5B1(&R[0], &R[1], &R[2], &R[3], &R[4]); /* stockham.py:566 */

    __syncthreads();
    if(write && thread < 15)
    {
        lds[offset_lds + ((thread / 5) * 25 + thread % 5 + 0) * lstride]
            = R[0]; /* stockham.py:577 */
        lds[offset_lds + ((thread / 5) * 25 + thread % 5 + 5) * lstride]
            = R[1]; /* stockham.py:577 */
        lds[offset_lds + ((thread / 5) * 25 + thread % 5 + 10) * lstride]
            = R[2]; /* stockham.py:577 */
        lds[offset_lds + ((thread / 5) * 25 + thread % 5 + 15) * lstride]
            = R[3]; /* stockham.py:577 */
        lds[offset_lds + ((thread / 5) * 25 + thread % 5 + 20) * lstride]
            = R[4]; /* stockham.py:577 */
    }

    // pass 2
    __syncthreads();
    R[0] = lds[offset_lds + (thread + 0) * lstride]; /* stockham.py:542 */
    R[1] = lds[offset_lds + (thread + 25) * lstride]; /* stockham.py:542 */
    R[2] = lds[offset_lds + (thread + 50) * lstride]; /* stockham.py:542 */

    W    = twiddles[24 + 2 * (thread % 25)]; /* stockham.py:554 */
    t.x  = W.x * R[1].x - W.y * R[1].y; /* stockham.py:555 */
    t.y  = W.y * R[1].x + W.x * R[1].y; /* stockham.py:556 */
    R[1] = t; /* stockham.py:557 */
    W    = twiddles[25 + 2 * (thread % 25)]; /* stockham.py:554 */
    t.x  = W.x * R[2].x - W.y * R[2].y; /* stockham.py:555 */
    t.y  = W.y * R[2].x + W.x * R[2].y; /* stockham.py:556 */
    R[2] = t; /* stockham.py:557 */

    FwdRad3B1(&R[0], &R[1], &R[2]); /* stockham.py:566 */

    __syncthreads();
    if(write)
    {
        lds[offset_lds + ((thread / 25) * 75 + thread % 25 + 0) * lstride]
            = R[0]; /* stockham.py:577 */
        lds[offset_lds + ((thread / 25) * 75 + thread % 25 + 25) * lstride]
            = R[1]; /* stockham.py:577 */
        lds[offset_lds + ((thread / 25) * 75 + thread % 25 + 50) * lstride]
            = R[2]; /* stockham.py:577 */
    }
}
/* stockham.py:709 */
template <typename scalar_type, StrideBin sb, EmbeddedType ebtype, CallbackType cbtype>
__global__
    __launch_bounds__(250) void ip_forward_length75_SBRR(const scalar_type* __restrict__ twiddles,
                                                         const size_t dim,
                                                         const size_t* __restrict__ lengths,
                                                         const size_t* __restrict__ stride,
                                                         const size_t       nbatch,
                                                         const unsigned int lds_padding,
                                                         void* __restrict__ load_cb_fn,
                                                         void* __restrict__ load_cb_data,
                                                         uint32_t load_cb_lds_bytes,
                                                         void* __restrict__ store_cb_fn,
                                                         void* __restrict__ store_cb_data,
                                                         scalar_type* __restrict__ buf)
{
    // this kernel:
    //   uses 25 threads per transform
    //   does 10 transforms per thread block
    // therefore it should be called with 250 threads per thread block
    extern __shared__ unsigned char __align__(sizeof(scalar_type)) lds_uchar[];
    scalar_type* __restrict__ lds = reinterpret_cast<scalar_type*>(lds_uchar);
    size_t       offset           = 0;
    unsigned int offset_lds;
    size_t       stride_lds;
    size_t       batch;
    size_t       transform;
    size_t       thread;
    bool         write;
    const size_t stride0  = (sb == SB_UNIT) ? (1) : (stride[0]);
    auto         load_cb  = get_load_cb<scalar_type, cbtype>(load_cb_fn);
    auto         store_cb = get_store_cb<scalar_type, cbtype>(store_cb_fn);

    // large twiddles

    // offsets
    size_t remaining;
    size_t plength = 1;
    size_t index_along_d;
    transform = blockIdx.x * 10 + threadIdx.x / 25; /* stockham.py:363 */
    remaining = transform; /* stockham.py:364 */
    for(int d = 1; d < dim; ++d)
    {
        plength       = plength * lengths[d]; /* stockham.py:367 */
        index_along_d = remaining % lengths[d]; /* stockham.py:368 */
        remaining     = remaining / lengths[d]; /* stockham.py:369 */
        offset        = offset + index_along_d * stride[d]; /* stockham.py:370 */
    }
    batch      = transform / plength; /* stockham.py:371 */
    offset     = offset + batch * stride[dim]; /* stockham.py:372 */
    offset_lds = (75 + lds_padding) * (transform % 10); /* stockham.py:373 */

    if(batch >= nbatch)
    {
        return;
    }

    // load global
    thread = threadIdx.x % 25; /* stockham.py:384 */
    lds[offset_lds + thread + 0]
        = load_cb(buf, offset + (thread + 0) * stride0, load_cb_data, nullptr);
    ; /* stockham.py:387 */
    lds[offset_lds + thread + 25]
        = load_cb(buf, offset + (thread + 25) * stride0, load_cb_data, nullptr);
    ; /* stockham.py:387 */
    lds[offset_lds + thread + 50]
        = load_cb(buf, offset + (thread + 50) * stride0, load_cb_data, nullptr);
    ; /* stockham.py:387 */

    // append extra global loading for C2Real pre-process only
    if(ebtype == EmbeddedType::C2Real_PRE)
    {
        // use the last thread of each transform to load one more element per row
        if(thread == 24)
        {
            lds[offset_lds + thread + 50 + 1]
                = load_cb(buf, offset + (thread + 50 + 1) * stride0, load_cb_data, nullptr);
            ; /* stockham.py:395 */
        }
    }

    // handle even-length real to complex pre-process in lds before transform
    if(ebtype == EmbeddedType::C2Real_PRE)
    {
        __syncthreads();

        real_pre_process_kernel_inplace<scalar_type, false>(thread % 38 + 0,
                                                            75 - thread % 38 - 0,
                                                            38,
                                                            &lds[offset_lds],
                                                            0,
                                                            &twiddles[75]); /* stockham.py:338 */

        real_pre_process_kernel_inplace<scalar_type, false>(thread % 38 + 25,
                                                            75 - thread % 38 - 25,
                                                            38,
                                                            &lds[offset_lds],
                                                            0,
                                                            &twiddles[75]); /* stockham.py:338 */

        __syncthreads();
    }

    // transform
    write = true; /* stockham.py:681 */
    ip_forward_length75_SBRR_device<scalar_type, SB_UNIT>(
        lds, twiddles, stride_lds, offset_lds, write); /* stockham.py:686 */

    // handle even-length complex to real post-process in lds after transform
    if(ebtype == EmbeddedType::Real2C_POST)
    {
        __syncthreads();

        real_post_process_kernel_inplace<scalar_type, false>(thread % 38 + 0,
                                                             75 - thread % 38 - 0,
                                                             38,
                                                             &lds[offset_lds],
                                                             0,
                                                             &twiddles[75]); /* stockham.py:338 */

        real_post_process_kernel_inplace<scalar_type, false>(thread % 38 + 25,
                                                             75 - thread % 38 - 25,
                                                             38,
                                                             &lds[offset_lds],
                                                             0,
                                                             &twiddles[75]); /* stockham.py:338 */
    }

    // store global
    __syncthreads();
    store_cb(
        buf, offset + (thread + 0) * stride0, lds[offset_lds + thread + 0], store_cb_data, nullptr);
    store_cb(buf,
             offset + (thread + 25) * stride0,
             lds[offset_lds + thread + 25],
             store_cb_data,
             nullptr);
    store_cb(buf,
             offset + (thread + 50) * stride0,
             lds[offset_lds + thread + 50],
             store_cb_data,
             nullptr);

    // append extra global write for Real2C post-process only
    if(ebtype == EmbeddedType::Real2C_POST)
    {
        // use the last thread of each transform to write one more element per row
        if(thread == 24)
        {
            store_cb(buf,
                     offset + (thread + 50 + 1) * stride0,
                     lds[offset_lds + thread + 50 + 1],
                     store_cb_data,
                     nullptr);
        }
    }
}
/* stockham.py:709 */
template <typename scalar_type, StrideBin sb, EmbeddedType ebtype, CallbackType cbtype>
__global__ __launch_bounds__(250) void ip_forward_length75_SBRR(
    const scalar_type* __restrict__ twiddles,
    const size_t dim,
    const size_t* __restrict__ lengths,
    const size_t* __restrict__ stride,
    const size_t       nbatch,
    const unsigned int lds_padding,
    void* __restrict__ load_cb_fn,
    void* __restrict__ load_cb_data,
    uint32_t load_cb_lds_bytes,
    void* __restrict__ store_cb_fn,
    void* __restrict__ store_cb_data,
    real_type_t<scalar_type>* __restrict__ bufre,
    real_type_t<scalar_type>* __restrict__ bufim)
{
    // this kernel:
    //   uses 25 threads per transform
    //   does 10 transforms per thread block
    // therefore it should be called with 250 threads per thread block
    extern __shared__ unsigned char __align__(sizeof(scalar_type)) lds_uchar[];
    scalar_type* __restrict__ lds = reinterpret_cast<scalar_type*>(lds_uchar);
    size_t       offset           = 0;
    unsigned int offset_lds;
    size_t       stride_lds;
    size_t       batch;
    size_t       transform;
    size_t       thread;
    bool         write;
    const size_t stride0  = (sb == SB_UNIT) ? (1) : (stride[0]);
    auto         load_cb  = get_load_cb<scalar_type, cbtype>(load_cb_fn);
    auto         store_cb = get_store_cb<scalar_type, cbtype>(store_cb_fn);

    // large twiddles

    // offsets
    size_t remaining;
    size_t plength = 1;
    size_t index_along_d;
    transform = blockIdx.x * 10 + threadIdx.x / 25; /* stockham.py:363 */
    remaining = transform; /* stockham.py:364 */
    for(int d = 1; d < dim; ++d)
    {
        plength       = plength * lengths[d]; /* stockham.py:367 */
        index_along_d = remaining % lengths[d]; /* stockham.py:368 */
        remaining     = remaining / lengths[d]; /* stockham.py:369 */
        offset        = offset + index_along_d * stride[d]; /* stockham.py:370 */
    }
    batch      = transform / plength; /* stockham.py:371 */
    offset     = offset + batch * stride[dim]; /* stockham.py:372 */
    offset_lds = (75 + lds_padding) * (transform % 10); /* stockham.py:373 */

    if(batch >= nbatch)
    {
        return;
    }

    // load global
    thread                       = threadIdx.x % 25; /* stockham.py:384 */
    lds[offset_lds + thread + 0] = {bufre[offset + (thread + 0) * stride0],
                                    bufim[offset + (thread + 0) * stride0]}; /* generator.py:872 */
    lds[offset_lds + thread + 25]
        = {bufre[offset + (thread + 25) * stride0],
           bufim[offset + (thread + 25) * stride0]}; /* generator.py:872 */
    lds[offset_lds + thread + 50]
        = {bufre[offset + (thread + 50) * stride0],
           bufim[offset + (thread + 50) * stride0]}; /* generator.py:872 */

    // append extra global loading for C2Real pre-process only
    if(ebtype == EmbeddedType::C2Real_PRE)
    {
        // use the last thread of each transform to load one more element per row
        if(thread == 24)
        {
            lds[offset_lds + thread + 50 + 1]
                = {bufre[offset + (thread + 50 + 1) * stride0],
                   bufim[offset + (thread + 50 + 1) * stride0]}; /* generator.py:872 */
        }
    }

    // handle even-length real to complex pre-process in lds before transform
    if(ebtype == EmbeddedType::C2Real_PRE)
    {
        __syncthreads();

        real_pre_process_kernel_inplace<scalar_type, false>(thread % 38 + 0,
                                                            75 - thread % 38 - 0,
                                                            38,
                                                            &lds[offset_lds],
                                                            0,
                                                            &twiddles[75]); /* stockham.py:338 */

        real_pre_process_kernel_inplace<scalar_type, false>(thread % 38 + 25,
                                                            75 - thread % 38 - 25,
                                                            38,
                                                            &lds[offset_lds],
                                                            0,
                                                            &twiddles[75]); /* stockham.py:338 */

        __syncthreads();
    }

    // transform
    write = true; /* stockham.py:681 */
    ip_forward_length75_SBRR_device<scalar_type, SB_UNIT>(
        lds, twiddles, stride_lds, offset_lds, write); /* stockham.py:686 */

    // handle even-length complex to real post-process in lds after transform
    if(ebtype == EmbeddedType::Real2C_POST)
    {
        __syncthreads();

        real_post_process_kernel_inplace<scalar_type, false>(thread % 38 + 0,
                                                             75 - thread % 38 - 0,
                                                             38,
                                                             &lds[offset_lds],
                                                             0,
                                                             &twiddles[75]); /* stockham.py:338 */

        real_post_process_kernel_inplace<scalar_type, false>(thread % 38 + 25,
                                                             75 - thread % 38 - 25,
                                                             38,
                                                             &lds[offset_lds],
                                                             0,
                                                             &twiddles[75]); /* stockham.py:338 */
    }

    // store global
    __syncthreads();
    bufre[offset + (thread + 0) * stride0] = lds[offset_lds + thread + 0].x; /* generator.py:906 */
    bufim[offset + (thread + 0) * stride0] = lds[offset_lds + thread + 0].y; /* generator.py:908 */
    bufre[offset + (thread + 25) * stride0]
        = lds[offset_lds + thread + 25].x; /* generator.py:906 */
    bufim[offset + (thread + 25) * stride0]
        = lds[offset_lds + thread + 25].y; /* generator.py:908 */
    bufre[offset + (thread + 50) * stride0]
        = lds[offset_lds + thread + 50].x; /* generator.py:906 */
    bufim[offset + (thread + 50) * stride0]
        = lds[offset_lds + thread + 50].y; /* generator.py:908 */

    // append extra global write for Real2C post-process only
    if(ebtype == EmbeddedType::Real2C_POST)
    {
        // use the last thread of each transform to write one more element per row
        if(thread == 24)
        {
            bufre[offset + (thread + 50 + 1) * stride0]
                = lds[offset_lds + thread + 50 + 1].x; /* generator.py:906 */
            bufim[offset + (thread + 50 + 1) * stride0]
                = lds[offset_lds + thread + 50 + 1].y; /* generator.py:908 */
        }
    }
}
/* stockham.py:765 */
template <typename scalar_type, StrideBin sb>
__device__ void op_forward_length75_SBRR_device(scalar_type* __restrict__ lds,
                                                const scalar_type* __restrict__ twiddles,
                                                size_t       stride_lds,
                                                unsigned int offset_lds,
                                                bool         write)
{
    size_t       thread;
    scalar_type  R[5];
    scalar_type  W;
    scalar_type  t;
    const size_t lstride = (sb == SB_UNIT) ? (1) : (stride_lds);
    thread               = threadIdx.x % 25; /* stockham.py:731 */

    // pass 0
    __syncthreads();
    R[0] = lds[offset_lds + (thread + 0) * lstride]; /* stockham.py:542 */
    R[1] = lds[offset_lds + (thread + 15) * lstride]; /* stockham.py:542 */
    R[2] = lds[offset_lds + (thread + 30) * lstride]; /* stockham.py:542 */
    R[3] = lds[offset_lds + (thread + 45) * lstride]; /* stockham.py:542 */
    R[4] = lds[offset_lds + (thread + 60) * lstride]; /* stockham.py:542 */

    FwdRad5B1(&R[0], &R[1], &R[2], &R[3], &R[4]); /* stockham.py:566 */

    __syncthreads();
    if(write && thread < 15)
    {
        lds[offset_lds + ((thread / 1) * 5 + thread % 1 + 0) * lstride]
            = R[0]; /* stockham.py:577 */
        lds[offset_lds + ((thread / 1) * 5 + thread % 1 + 1) * lstride]
            = R[1]; /* stockham.py:577 */
        lds[offset_lds + ((thread / 1) * 5 + thread % 1 + 2) * lstride]
            = R[2]; /* stockham.py:577 */
        lds[offset_lds + ((thread / 1) * 5 + thread % 1 + 3) * lstride]
            = R[3]; /* stockham.py:577 */
        lds[offset_lds + ((thread / 1) * 5 + thread % 1 + 4) * lstride]
            = R[4]; /* stockham.py:577 */
    }

    // pass 1
    __syncthreads();
    R[0] = lds[offset_lds + (thread + 0) * lstride]; /* stockham.py:542 */
    R[1] = lds[offset_lds + (thread + 15) * lstride]; /* stockham.py:542 */
    R[2] = lds[offset_lds + (thread + 30) * lstride]; /* stockham.py:542 */
    R[3] = lds[offset_lds + (thread + 45) * lstride]; /* stockham.py:542 */
    R[4] = lds[offset_lds + (thread + 60) * lstride]; /* stockham.py:542 */

    W    = twiddles[4 + 4 * (thread % 5)]; /* stockham.py:554 */
    t.x  = W.x * R[1].x - W.y * R[1].y; /* stockham.py:555 */
    t.y  = W.y * R[1].x + W.x * R[1].y; /* stockham.py:556 */
    R[1] = t; /* stockham.py:557 */
    W    = twiddles[5 + 4 * (thread % 5)]; /* stockham.py:554 */
    t.x  = W.x * R[2].x - W.y * R[2].y; /* stockham.py:555 */
    t.y  = W.y * R[2].x + W.x * R[2].y; /* stockham.py:556 */
    R[2] = t; /* stockham.py:557 */
    W    = twiddles[6 + 4 * (thread % 5)]; /* stockham.py:554 */
    t.x  = W.x * R[3].x - W.y * R[3].y; /* stockham.py:555 */
    t.y  = W.y * R[3].x + W.x * R[3].y; /* stockham.py:556 */
    R[3] = t; /* stockham.py:557 */
    W    = twiddles[7 + 4 * (thread % 5)]; /* stockham.py:554 */
    t.x  = W.x * R[4].x - W.y * R[4].y; /* stockham.py:555 */
    t.y  = W.y * R[4].x + W.x * R[4].y; /* stockham.py:556 */
    R[4] = t; /* stockham.py:557 */

    FwdRad5B1(&R[0], &R[1], &R[2], &R[3], &R[4]); /* stockham.py:566 */

    __syncthreads();
    if(write && thread < 15)
    {
        lds[offset_lds + ((thread / 5) * 25 + thread % 5 + 0) * lstride]
            = R[0]; /* stockham.py:577 */
        lds[offset_lds + ((thread / 5) * 25 + thread % 5 + 5) * lstride]
            = R[1]; /* stockham.py:577 */
        lds[offset_lds + ((thread / 5) * 25 + thread % 5 + 10) * lstride]
            = R[2]; /* stockham.py:577 */
        lds[offset_lds + ((thread / 5) * 25 + thread % 5 + 15) * lstride]
            = R[3]; /* stockham.py:577 */
        lds[offset_lds + ((thread / 5) * 25 + thread % 5 + 20) * lstride]
            = R[4]; /* stockham.py:577 */
    }

    // pass 2
    __syncthreads();
    R[0] = lds[offset_lds + (thread + 0) * lstride]; /* stockham.py:542 */
    R[1] = lds[offset_lds + (thread + 25) * lstride]; /* stockham.py:542 */
    R[2] = lds[offset_lds + (thread + 50) * lstride]; /* stockham.py:542 */

    W    = twiddles[24 + 2 * (thread % 25)]; /* stockham.py:554 */
    t.x  = W.x * R[1].x - W.y * R[1].y; /* stockham.py:555 */
    t.y  = W.y * R[1].x + W.x * R[1].y; /* stockham.py:556 */
    R[1] = t; /* stockham.py:557 */
    W    = twiddles[25 + 2 * (thread % 25)]; /* stockham.py:554 */
    t.x  = W.x * R[2].x - W.y * R[2].y; /* stockham.py:555 */
    t.y  = W.y * R[2].x + W.x * R[2].y; /* stockham.py:556 */
    R[2] = t; /* stockham.py:557 */

    FwdRad3B1(&R[0], &R[1], &R[2]); /* stockham.py:566 */

    __syncthreads();
    if(write)
    {
        lds[offset_lds + ((thread / 25) * 75 + thread % 25 + 0) * lstride]
            = R[0]; /* stockham.py:577 */
        lds[offset_lds + ((thread / 25) * 75 + thread % 25 + 25) * lstride]
            = R[1]; /* stockham.py:577 */
        lds[offset_lds + ((thread / 25) * 75 + thread % 25 + 50) * lstride]
            = R[2]; /* stockham.py:577 */
    }
}
/* stockham.py:709 */
template <typename scalar_type, StrideBin sb, EmbeddedType ebtype, CallbackType cbtype>
__global__
    __launch_bounds__(250) void op_forward_length75_SBRR(const scalar_type* __restrict__ twiddles,
                                                         const size_t dim,
                                                         const size_t* __restrict__ lengths,
                                                         const size_t* __restrict__ stride_in,
                                                         const size_t* __restrict__ stride_out,
                                                         const size_t       nbatch,
                                                         const unsigned int lds_padding,
                                                         void* __restrict__ load_cb_fn,
                                                         void* __restrict__ load_cb_data,
                                                         uint32_t load_cb_lds_bytes,
                                                         void* __restrict__ store_cb_fn,
                                                         void* __restrict__ store_cb_data,
                                                         scalar_type* __restrict__ buf_in,
                                                         scalar_type* __restrict__ buf_out)
{
    // this kernel:
    //   uses 25 threads per transform
    //   does 10 transforms per thread block
    // therefore it should be called with 250 threads per thread block
    extern __shared__ unsigned char __align__(sizeof(scalar_type)) lds_uchar[];
    scalar_type* __restrict__ lds = reinterpret_cast<scalar_type*>(lds_uchar);
    size_t       offset_in        = 0;
    size_t       offset_out       = 0;
    unsigned int offset_lds;
    size_t       stride_lds;
    size_t       batch;
    size_t       transform;
    size_t       thread;
    bool         write;
    const size_t stride0_in  = (sb == SB_UNIT) ? (1) : (stride_in[0]);
    const size_t stride0_out = (sb == SB_UNIT) ? (1) : (stride_out[0]);
    auto         load_cb     = get_load_cb<scalar_type, cbtype>(load_cb_fn);
    auto         store_cb    = get_store_cb<scalar_type, cbtype>(store_cb_fn);

    // large twiddles

    // offsets
    size_t remaining;
    size_t plength = 1;
    size_t index_along_d;
    transform = blockIdx.x * 10 + threadIdx.x / 25; /* stockham.py:363 */
    remaining = transform; /* stockham.py:364 */
    for(int d = 1; d < dim; ++d)
    {
        plength       = plength * lengths[d]; /* stockham.py:367 */
        index_along_d = remaining % lengths[d]; /* stockham.py:368 */
        remaining     = remaining / lengths[d]; /* stockham.py:369 */
        offset_in     = offset_in + index_along_d * stride_in[d]; /* generator.py:992 */
        offset_out    = offset_out + index_along_d * stride_out[d]; /* generator.py:993 */
    }
    batch      = transform / plength; /* stockham.py:371 */
    offset_in  = offset_in + batch * stride_in[dim]; /* generator.py:992 */
    offset_out = offset_out + batch * stride_out[dim]; /* generator.py:993 */
    offset_lds = (75 + lds_padding) * (transform % 10); /* stockham.py:373 */

    if(batch >= nbatch)
    {
        return;
    }

    // load global
    thread = threadIdx.x % 25; /* stockham.py:384 */
    lds[offset_lds + thread + 0]
        = load_cb(buf_in, offset_in + (thread + 0) * stride0_in, load_cb_data, nullptr);
    ; /* stockham.py:387 */
    lds[offset_lds + thread + 25]
        = load_cb(buf_in, offset_in + (thread + 25) * stride0_in, load_cb_data, nullptr);
    ; /* stockham.py:387 */
    lds[offset_lds + thread + 50]
        = load_cb(buf_in, offset_in + (thread + 50) * stride0_in, load_cb_data, nullptr);
    ; /* stockham.py:387 */

    // append extra global loading for C2Real pre-process only
    if(ebtype == EmbeddedType::C2Real_PRE)
    {
        // use the last thread of each transform to load one more element per row
        if(thread == 24)
        {
            lds[offset_lds + thread + 50 + 1] = load_cb(
                buf_in, offset_in + (thread + 50 + 1) * stride0_in, load_cb_data, nullptr);
            ; /* stockham.py:395 */
        }
    }

    // handle even-length real to complex pre-process in lds before transform
    if(ebtype == EmbeddedType::C2Real_PRE)
    {
        __syncthreads();

        real_pre_process_kernel_inplace<scalar_type, false>(thread % 38 + 0,
                                                            75 - thread % 38 - 0,
                                                            38,
                                                            &lds[offset_lds],
                                                            0,
                                                            &twiddles[75]); /* stockham.py:338 */

        real_pre_process_kernel_inplace<scalar_type, false>(thread % 38 + 25,
                                                            75 - thread % 38 - 25,
                                                            38,
                                                            &lds[offset_lds],
                                                            0,
                                                            &twiddles[75]); /* stockham.py:338 */

        __syncthreads();
    }

    // transform
    write = true; /* stockham.py:681 */
    op_forward_length75_SBRR_device<scalar_type, SB_UNIT>(
        lds, twiddles, stride_lds, offset_lds, write); /* stockham.py:686 */

    // handle even-length complex to real post-process in lds after transform
    if(ebtype == EmbeddedType::Real2C_POST)
    {
        __syncthreads();

        real_post_process_kernel_inplace<scalar_type, false>(thread % 38 + 0,
                                                             75 - thread % 38 - 0,
                                                             38,
                                                             &lds[offset_lds],
                                                             0,
                                                             &twiddles[75]); /* stockham.py:338 */

        real_post_process_kernel_inplace<scalar_type, false>(thread % 38 + 25,
                                                             75 - thread % 38 - 25,
                                                             38,
                                                             &lds[offset_lds],
                                                             0,
                                                             &twiddles[75]); /* stockham.py:338 */
    }

    // store global
    __syncthreads();
    store_cb(buf_out,
             offset_out + (thread + 0) * stride0_out,
             lds[offset_lds + thread + 0],
             store_cb_data,
             nullptr);
    store_cb(buf_out,
             offset_out + (thread + 25) * stride0_out,
             lds[offset_lds + thread + 25],
             store_cb_data,
             nullptr);
    store_cb(buf_out,
             offset_out + (thread + 50) * stride0_out,
             lds[offset_lds + thread + 50],
             store_cb_data,
             nullptr);

    // append extra global write for Real2C post-process only
    if(ebtype == EmbeddedType::Real2C_POST)
    {
        // use the last thread of each transform to write one more element per row
        if(thread == 24)
        {
            store_cb(buf_out,
                     offset_out + (thread + 50 + 1) * stride0_out,
                     lds[offset_lds + thread + 50 + 1],
                     store_cb_data,
                     nullptr);
        }
    }
}
/* stockham.py:709 */
template <typename scalar_type, StrideBin sb, EmbeddedType ebtype, CallbackType cbtype>
__global__ __launch_bounds__(250) void op_forward_length75_SBRR(
    const scalar_type* __restrict__ twiddles,
    const size_t dim,
    const size_t* __restrict__ lengths,
    const size_t* __restrict__ stride_in,
    const size_t* __restrict__ stride_out,
    const size_t       nbatch,
    const unsigned int lds_padding,
    void* __restrict__ load_cb_fn,
    void* __restrict__ load_cb_data,
    uint32_t load_cb_lds_bytes,
    void* __restrict__ store_cb_fn,
    void* __restrict__ store_cb_data,
    scalar_type* __restrict__ buf_in,
    real_type_t<scalar_type>* __restrict__ buf_outre,
    real_type_t<scalar_type>* __restrict__ buf_outim)
{
    // this kernel:
    //   uses 25 threads per transform
    //   does 10 transforms per thread block
    // therefore it should be called with 250 threads per thread block
    extern __shared__ unsigned char __align__(sizeof(scalar_type)) lds_uchar[];
    scalar_type* __restrict__ lds = reinterpret_cast<scalar_type*>(lds_uchar);
    size_t       offset_in        = 0;
    size_t       offset_out       = 0;
    unsigned int offset_lds;
    size_t       stride_lds;
    size_t       batch;
    size_t       transform;
    size_t       thread;
    bool         write;
    const size_t stride0_in  = (sb == SB_UNIT) ? (1) : (stride_in[0]);
    const size_t stride0_out = (sb == SB_UNIT) ? (1) : (stride_out[0]);
    auto         load_cb     = get_load_cb<scalar_type, cbtype>(load_cb_fn);
    auto         store_cb    = get_store_cb<scalar_type, cbtype>(store_cb_fn);

    // large twiddles

    // offsets
    size_t remaining;
    size_t plength = 1;
    size_t index_along_d;
    transform = blockIdx.x * 10 + threadIdx.x / 25; /* stockham.py:363 */
    remaining = transform; /* stockham.py:364 */
    for(int d = 1; d < dim; ++d)
    {
        plength       = plength * lengths[d]; /* stockham.py:367 */
        index_along_d = remaining % lengths[d]; /* stockham.py:368 */
        remaining     = remaining / lengths[d]; /* stockham.py:369 */
        offset_in     = offset_in + index_along_d * stride_in[d]; /* generator.py:992 */
        offset_out    = offset_out + index_along_d * stride_out[d]; /* generator.py:993 */
    }
    batch      = transform / plength; /* stockham.py:371 */
    offset_in  = offset_in + batch * stride_in[dim]; /* generator.py:992 */
    offset_out = offset_out + batch * stride_out[dim]; /* generator.py:993 */
    offset_lds = (75 + lds_padding) * (transform % 10); /* stockham.py:373 */

    if(batch >= nbatch)
    {
        return;
    }

    // load global
    thread = threadIdx.x % 25; /* stockham.py:384 */
    lds[offset_lds + thread + 0]
        = load_cb(buf_in, offset_in + (thread + 0) * stride0_in, load_cb_data, nullptr);
    ; /* stockham.py:387 */
    lds[offset_lds + thread + 25]
        = load_cb(buf_in, offset_in + (thread + 25) * stride0_in, load_cb_data, nullptr);
    ; /* stockham.py:387 */
    lds[offset_lds + thread + 50]
        = load_cb(buf_in, offset_in + (thread + 50) * stride0_in, load_cb_data, nullptr);
    ; /* stockham.py:387 */

    // append extra global loading for C2Real pre-process only
    if(ebtype == EmbeddedType::C2Real_PRE)
    {
        // use the last thread of each transform to load one more element per row
        if(thread == 24)
        {
            lds[offset_lds + thread + 50 + 1] = load_cb(
                buf_in, offset_in + (thread + 50 + 1) * stride0_in, load_cb_data, nullptr);
            ; /* stockham.py:395 */
        }
    }

    // handle even-length real to complex pre-process in lds before transform
    if(ebtype == EmbeddedType::C2Real_PRE)
    {
        __syncthreads();

        real_pre_process_kernel_inplace<scalar_type, false>(thread % 38 + 0,
                                                            75 - thread % 38 - 0,
                                                            38,
                                                            &lds[offset_lds],
                                                            0,
                                                            &twiddles[75]); /* stockham.py:338 */

        real_pre_process_kernel_inplace<scalar_type, false>(thread % 38 + 25,
                                                            75 - thread % 38 - 25,
                                                            38,
                                                            &lds[offset_lds],
                                                            0,
                                                            &twiddles[75]); /* stockham.py:338 */

        __syncthreads();
    }

    // transform
    write = true; /* stockham.py:681 */
    op_forward_length75_SBRR_device<scalar_type, SB_UNIT>(
        lds, twiddles, stride_lds, offset_lds, write); /* stockham.py:686 */

    // handle even-length complex to real post-process in lds after transform
    if(ebtype == EmbeddedType::Real2C_POST)
    {
        __syncthreads();

        real_post_process_kernel_inplace<scalar_type, false>(thread % 38 + 0,
                                                             75 - thread % 38 - 0,
                                                             38,
                                                             &lds[offset_lds],
                                                             0,
                                                             &twiddles[75]); /* stockham.py:338 */

        real_post_process_kernel_inplace<scalar_type, false>(thread % 38 + 25,
                                                             75 - thread % 38 - 25,
                                                             38,
                                                             &lds[offset_lds],
                                                             0,
                                                             &twiddles[75]); /* stockham.py:338 */
    }

    // store global
    __syncthreads();
    buf_outre[offset_out + (thread + 0) * stride0_out]
        = lds[offset_lds + thread + 0].x; /* generator.py:906 */
    buf_outim[offset_out + (thread + 0) * stride0_out]
        = lds[offset_lds + thread + 0].y; /* generator.py:908 */
    buf_outre[offset_out + (thread + 25) * stride0_out]
        = lds[offset_lds + thread + 25].x; /* generator.py:906 */
    buf_outim[offset_out + (thread + 25) * stride0_out]
        = lds[offset_lds + thread + 25].y; /* generator.py:908 */
    buf_outre[offset_out + (thread + 50) * stride0_out]
        = lds[offset_lds + thread + 50].x; /* generator.py:906 */
    buf_outim[offset_out + (thread + 50) * stride0_out]
        = lds[offset_lds + thread + 50].y; /* generator.py:908 */

    // append extra global write for Real2C post-process only
    if(ebtype == EmbeddedType::Real2C_POST)
    {
        // use the last thread of each transform to write one more element per row
        if(thread == 24)
        {
            buf_outre[offset_out + (thread + 50 + 1) * stride0_out]
                = lds[offset_lds + thread + 50 + 1].x; /* generator.py:906 */
            buf_outim[offset_out + (thread + 50 + 1) * stride0_out]
                = lds[offset_lds + thread + 50 + 1].y; /* generator.py:908 */
        }
    }
}
/* stockham.py:709 */
template <typename scalar_type, StrideBin sb, EmbeddedType ebtype, CallbackType cbtype>
__global__ __launch_bounds__(250) void op_forward_length75_SBRR(
    const scalar_type* __restrict__ twiddles,
    const size_t dim,
    const size_t* __restrict__ lengths,
    const size_t* __restrict__ stride_in,
    const size_t* __restrict__ stride_out,
    const size_t       nbatch,
    const unsigned int lds_padding,
    void* __restrict__ load_cb_fn,
    void* __restrict__ load_cb_data,
    uint32_t load_cb_lds_bytes,
    void* __restrict__ store_cb_fn,
    void* __restrict__ store_cb_data,
    real_type_t<scalar_type>* __restrict__ buf_inre,
    real_type_t<scalar_type>* __restrict__ buf_inim,
    scalar_type* __restrict__ buf_out)
{
    // this kernel:
    //   uses 25 threads per transform
    //   does 10 transforms per thread block
    // therefore it should be called with 250 threads per thread block
    extern __shared__ unsigned char __align__(sizeof(scalar_type)) lds_uchar[];
    scalar_type* __restrict__ lds = reinterpret_cast<scalar_type*>(lds_uchar);
    size_t       offset_in        = 0;
    size_t       offset_out       = 0;
    unsigned int offset_lds;
    size_t       stride_lds;
    size_t       batch;
    size_t       transform;
    size_t       thread;
    bool         write;
    const size_t stride0_in  = (sb == SB_UNIT) ? (1) : (stride_in[0]);
    const size_t stride0_out = (sb == SB_UNIT) ? (1) : (stride_out[0]);
    auto         load_cb     = get_load_cb<scalar_type, cbtype>(load_cb_fn);
    auto         store_cb    = get_store_cb<scalar_type, cbtype>(store_cb_fn);

    // large twiddles

    // offsets
    size_t remaining;
    size_t plength = 1;
    size_t index_along_d;
    transform = blockIdx.x * 10 + threadIdx.x / 25; /* stockham.py:363 */
    remaining = transform; /* stockham.py:364 */
    for(int d = 1; d < dim; ++d)
    {
        plength       = plength * lengths[d]; /* stockham.py:367 */
        index_along_d = remaining % lengths[d]; /* stockham.py:368 */
        remaining     = remaining / lengths[d]; /* stockham.py:369 */
        offset_in     = offset_in + index_along_d * stride_in[d]; /* generator.py:992 */
        offset_out    = offset_out + index_along_d * stride_out[d]; /* generator.py:993 */
    }
    batch      = transform / plength; /* stockham.py:371 */
    offset_in  = offset_in + batch * stride_in[dim]; /* generator.py:992 */
    offset_out = offset_out + batch * stride_out[dim]; /* generator.py:993 */
    offset_lds = (75 + lds_padding) * (transform % 10); /* stockham.py:373 */

    if(batch >= nbatch)
    {
        return;
    }

    // load global
    thread = threadIdx.x % 25; /* stockham.py:384 */
    lds[offset_lds + thread + 0]
        = {buf_inre[offset_in + (thread + 0) * stride0_in],
           buf_inim[offset_in + (thread + 0) * stride0_in]}; /* generator.py:872 */
    lds[offset_lds + thread + 25]
        = {buf_inre[offset_in + (thread + 25) * stride0_in],
           buf_inim[offset_in + (thread + 25) * stride0_in]}; /* generator.py:872 */
    lds[offset_lds + thread + 50]
        = {buf_inre[offset_in + (thread + 50) * stride0_in],
           buf_inim[offset_in + (thread + 50) * stride0_in]}; /* generator.py:872 */

    // append extra global loading for C2Real pre-process only
    if(ebtype == EmbeddedType::C2Real_PRE)
    {
        // use the last thread of each transform to load one more element per row
        if(thread == 24)
        {
            lds[offset_lds + thread + 50 + 1]
                = {buf_inre[offset_in + (thread + 50 + 1) * stride0_in],
                   buf_inim[offset_in + (thread + 50 + 1) * stride0_in]}; /* generator.py:872 */
        }
    }

    // handle even-length real to complex pre-process in lds before transform
    if(ebtype == EmbeddedType::C2Real_PRE)
    {
        __syncthreads();

        real_pre_process_kernel_inplace<scalar_type, false>(thread % 38 + 0,
                                                            75 - thread % 38 - 0,
                                                            38,
                                                            &lds[offset_lds],
                                                            0,
                                                            &twiddles[75]); /* stockham.py:338 */

        real_pre_process_kernel_inplace<scalar_type, false>(thread % 38 + 25,
                                                            75 - thread % 38 - 25,
                                                            38,
                                                            &lds[offset_lds],
                                                            0,
                                                            &twiddles[75]); /* stockham.py:338 */

        __syncthreads();
    }

    // transform
    write = true; /* stockham.py:681 */
    op_forward_length75_SBRR_device<scalar_type, SB_UNIT>(
        lds, twiddles, stride_lds, offset_lds, write); /* stockham.py:686 */

    // handle even-length complex to real post-process in lds after transform
    if(ebtype == EmbeddedType::Real2C_POST)
    {
        __syncthreads();

        real_post_process_kernel_inplace<scalar_type, false>(thread % 38 + 0,
                                                             75 - thread % 38 - 0,
                                                             38,
                                                             &lds[offset_lds],
                                                             0,
                                                             &twiddles[75]); /* stockham.py:338 */

        real_post_process_kernel_inplace<scalar_type, false>(thread % 38 + 25,
                                                             75 - thread % 38 - 25,
                                                             38,
                                                             &lds[offset_lds],
                                                             0,
                                                             &twiddles[75]); /* stockham.py:338 */
    }

    // store global
    __syncthreads();
    store_cb(buf_out,
             offset_out + (thread + 0) * stride0_out,
             lds[offset_lds + thread + 0],
             store_cb_data,
             nullptr);
    store_cb(buf_out,
             offset_out + (thread + 25) * stride0_out,
             lds[offset_lds + thread + 25],
             store_cb_data,
             nullptr);
    store_cb(buf_out,
             offset_out + (thread + 50) * stride0_out,
             lds[offset_lds + thread + 50],
             store_cb_data,
             nullptr);

    // append extra global write for Real2C post-process only
    if(ebtype == EmbeddedType::Real2C_POST)
    {
        // use the last thread of each transform to write one more element per row
        if(thread == 24)
        {
            store_cb(buf_out,
                     offset_out + (thread + 50 + 1) * stride0_out,
                     lds[offset_lds + thread + 50 + 1],
                     store_cb_data,
                     nullptr);
        }
    }
}
/* stockham.py:709 */
template <typename scalar_type, StrideBin sb, EmbeddedType ebtype, CallbackType cbtype>
__global__ __launch_bounds__(250) void op_forward_length75_SBRR(
    const scalar_type* __restrict__ twiddles,
    const size_t dim,
    const size_t* __restrict__ lengths,
    const size_t* __restrict__ stride_in,
    const size_t* __restrict__ stride_out,
    const size_t       nbatch,
    const unsigned int lds_padding,
    void* __restrict__ load_cb_fn,
    void* __restrict__ load_cb_data,
    uint32_t load_cb_lds_bytes,
    void* __restrict__ store_cb_fn,
    void* __restrict__ store_cb_data,
    real_type_t<scalar_type>* __restrict__ buf_inre,
    real_type_t<scalar_type>* __restrict__ buf_inim,
    real_type_t<scalar_type>* __restrict__ buf_outre,
    real_type_t<scalar_type>* __restrict__ buf_outim)
{
    // this kernel:
    //   uses 25 threads per transform
    //   does 10 transforms per thread block
    // therefore it should be called with 250 threads per thread block
    extern __shared__ unsigned char __align__(sizeof(scalar_type)) lds_uchar[];
    scalar_type* __restrict__ lds = reinterpret_cast<scalar_type*>(lds_uchar);
    size_t       offset_in        = 0;
    size_t       offset_out       = 0;
    unsigned int offset_lds;
    size_t       stride_lds;
    size_t       batch;
    size_t       transform;
    size_t       thread;
    bool         write;
    const size_t stride0_in  = (sb == SB_UNIT) ? (1) : (stride_in[0]);
    const size_t stride0_out = (sb == SB_UNIT) ? (1) : (stride_out[0]);
    auto         load_cb     = get_load_cb<scalar_type, cbtype>(load_cb_fn);
    auto         store_cb    = get_store_cb<scalar_type, cbtype>(store_cb_fn);

    // large twiddles

    // offsets
    size_t remaining;
    size_t plength = 1;
    size_t index_along_d;
    transform = blockIdx.x * 10 + threadIdx.x / 25; /* stockham.py:363 */
    remaining = transform; /* stockham.py:364 */
    for(int d = 1; d < dim; ++d)
    {
        plength       = plength * lengths[d]; /* stockham.py:367 */
        index_along_d = remaining % lengths[d]; /* stockham.py:368 */
        remaining     = remaining / lengths[d]; /* stockham.py:369 */
        offset_in     = offset_in + index_along_d * stride_in[d]; /* generator.py:992 */
        offset_out    = offset_out + index_along_d * stride_out[d]; /* generator.py:993 */
    }
    batch      = transform / plength; /* stockham.py:371 */
    offset_in  = offset_in + batch * stride_in[dim]; /* generator.py:992 */
    offset_out = offset_out + batch * stride_out[dim]; /* generator.py:993 */
    offset_lds = (75 + lds_padding) * (transform % 10); /* stockham.py:373 */

    if(batch >= nbatch)
    {
        return;
    }

    // load global
    thread = threadIdx.x % 25; /* stockham.py:384 */
    lds[offset_lds + thread + 0]
        = {buf_inre[offset_in + (thread + 0) * stride0_in],
           buf_inim[offset_in + (thread + 0) * stride0_in]}; /* generator.py:872 */
    lds[offset_lds + thread + 25]
        = {buf_inre[offset_in + (thread + 25) * stride0_in],
           buf_inim[offset_in + (thread + 25) * stride0_in]}; /* generator.py:872 */
    lds[offset_lds + thread + 50]
        = {buf_inre[offset_in + (thread + 50) * stride0_in],
           buf_inim[offset_in + (thread + 50) * stride0_in]}; /* generator.py:872 */

    // append extra global loading for C2Real pre-process only
    if(ebtype == EmbeddedType::C2Real_PRE)
    {
        // use the last thread of each transform to load one more element per row
        if(thread == 24)
        {
            lds[offset_lds + thread + 50 + 1]
                = {buf_inre[offset_in + (thread + 50 + 1) * stride0_in],
                   buf_inim[offset_in + (thread + 50 + 1) * stride0_in]}; /* generator.py:872 */
        }
    }

    // handle even-length real to complex pre-process in lds before transform
    if(ebtype == EmbeddedType::C2Real_PRE)
    {
        __syncthreads();

        real_pre_process_kernel_inplace<scalar_type, false>(thread % 38 + 0,
                                                            75 - thread % 38 - 0,
                                                            38,
                                                            &lds[offset_lds],
                                                            0,
                                                            &twiddles[75]); /* stockham.py:338 */

        real_pre_process_kernel_inplace<scalar_type, false>(thread % 38 + 25,
                                                            75 - thread % 38 - 25,
                                                            38,
                                                            &lds[offset_lds],
                                                            0,
                                                            &twiddles[75]); /* stockham.py:338 */

        __syncthreads();
    }

    // transform
    write = true; /* stockham.py:681 */
    op_forward_length75_SBRR_device<scalar_type, SB_UNIT>(
        lds, twiddles, stride_lds, offset_lds, write); /* stockham.py:686 */

    // handle even-length complex to real post-process in lds after transform
    if(ebtype == EmbeddedType::Real2C_POST)
    {
        __syncthreads();

        real_post_process_kernel_inplace<scalar_type, false>(thread % 38 + 0,
                                                             75 - thread % 38 - 0,
                                                             38,
                                                             &lds[offset_lds],
                                                             0,
                                                             &twiddles[75]); /* stockham.py:338 */

        real_post_process_kernel_inplace<scalar_type, false>(thread % 38 + 25,
                                                             75 - thread % 38 - 25,
                                                             38,
                                                             &lds[offset_lds],
                                                             0,
                                                             &twiddles[75]); /* stockham.py:338 */
    }

    // store global
    __syncthreads();
    buf_outre[offset_out + (thread + 0) * stride0_out]
        = lds[offset_lds + thread + 0].x; /* generator.py:906 */
    buf_outim[offset_out + (thread + 0) * stride0_out]
        = lds[offset_lds + thread + 0].y; /* generator.py:908 */
    buf_outre[offset_out + (thread + 25) * stride0_out]
        = lds[offset_lds + thread + 25].x; /* generator.py:906 */
    buf_outim[offset_out + (thread + 25) * stride0_out]
        = lds[offset_lds + thread + 25].y; /* generator.py:908 */
    buf_outre[offset_out + (thread + 50) * stride0_out]
        = lds[offset_lds + thread + 50].x; /* generator.py:906 */
    buf_outim[offset_out + (thread + 50) * stride0_out]
        = lds[offset_lds + thread + 50].y; /* generator.py:908 */

    // append extra global write for Real2C post-process only
    if(ebtype == EmbeddedType::Real2C_POST)
    {
        // use the last thread of each transform to write one more element per row
        if(thread == 24)
        {
            buf_outre[offset_out + (thread + 50 + 1) * stride0_out]
                = lds[offset_lds + thread + 50 + 1].x; /* generator.py:906 */
            buf_outim[offset_out + (thread + 50 + 1) * stride0_out]
                = lds[offset_lds + thread + 50 + 1].y; /* generator.py:908 */
        }
    }
}
/* stockham.py:765 */
template <typename scalar_type, StrideBin sb>
__device__ void ip_inverse_length75_SBRR_device(scalar_type* __restrict__ lds,
                                                const scalar_type* __restrict__ twiddles,
                                                size_t       stride_lds,
                                                unsigned int offset_lds,
                                                bool         write)
{
    size_t       thread;
    scalar_type  R[5];
    scalar_type  W;
    scalar_type  t;
    const size_t lstride = (sb == SB_UNIT) ? (1) : (stride_lds);
    thread               = threadIdx.x % 25; /* stockham.py:731 */

    // pass 0
    __syncthreads();
    R[0] = lds[offset_lds + (thread + 0) * lstride]; /* stockham.py:542 */
    R[1] = lds[offset_lds + (thread + 15) * lstride]; /* stockham.py:542 */
    R[2] = lds[offset_lds + (thread + 30) * lstride]; /* stockham.py:542 */
    R[3] = lds[offset_lds + (thread + 45) * lstride]; /* stockham.py:542 */
    R[4] = lds[offset_lds + (thread + 60) * lstride]; /* stockham.py:542 */

    InvRad5B1(&R[0], &R[1], &R[2], &R[3], &R[4]); /* stockham.py:566 */

    __syncthreads();
    if(write && thread < 15)
    {
        lds[offset_lds + ((thread / 1) * 5 + thread % 1 + 0) * lstride]
            = R[0]; /* stockham.py:577 */
        lds[offset_lds + ((thread / 1) * 5 + thread % 1 + 1) * lstride]
            = R[1]; /* stockham.py:577 */
        lds[offset_lds + ((thread / 1) * 5 + thread % 1 + 2) * lstride]
            = R[2]; /* stockham.py:577 */
        lds[offset_lds + ((thread / 1) * 5 + thread % 1 + 3) * lstride]
            = R[3]; /* stockham.py:577 */
        lds[offset_lds + ((thread / 1) * 5 + thread % 1 + 4) * lstride]
            = R[4]; /* stockham.py:577 */
    }

    // pass 1
    __syncthreads();
    R[0] = lds[offset_lds + (thread + 0) * lstride]; /* stockham.py:542 */
    R[1] = lds[offset_lds + (thread + 15) * lstride]; /* stockham.py:542 */
    R[2] = lds[offset_lds + (thread + 30) * lstride]; /* stockham.py:542 */
    R[3] = lds[offset_lds + (thread + 45) * lstride]; /* stockham.py:542 */
    R[4] = lds[offset_lds + (thread + 60) * lstride]; /* stockham.py:542 */

    W    = {twiddles[4 + 4 * (thread % 5)].x,
         -twiddles[4 + 4 * (thread % 5)].y}; /* generator.py:1065 */
    t.x  = W.x * R[1].x - W.y * R[1].y; /* stockham.py:555 */
    t.y  = W.y * R[1].x + W.x * R[1].y; /* stockham.py:556 */
    R[1] = t; /* stockham.py:557 */
    W    = {twiddles[5 + 4 * (thread % 5)].x,
         -twiddles[5 + 4 * (thread % 5)].y}; /* generator.py:1065 */
    t.x  = W.x * R[2].x - W.y * R[2].y; /* stockham.py:555 */
    t.y  = W.y * R[2].x + W.x * R[2].y; /* stockham.py:556 */
    R[2] = t; /* stockham.py:557 */
    W    = {twiddles[6 + 4 * (thread % 5)].x,
         -twiddles[6 + 4 * (thread % 5)].y}; /* generator.py:1065 */
    t.x  = W.x * R[3].x - W.y * R[3].y; /* stockham.py:555 */
    t.y  = W.y * R[3].x + W.x * R[3].y; /* stockham.py:556 */
    R[3] = t; /* stockham.py:557 */
    W    = {twiddles[7 + 4 * (thread % 5)].x,
         -twiddles[7 + 4 * (thread % 5)].y}; /* generator.py:1065 */
    t.x  = W.x * R[4].x - W.y * R[4].y; /* stockham.py:555 */
    t.y  = W.y * R[4].x + W.x * R[4].y; /* stockham.py:556 */
    R[4] = t; /* stockham.py:557 */

    InvRad5B1(&R[0], &R[1], &R[2], &R[3], &R[4]); /* stockham.py:566 */

    __syncthreads();
    if(write && thread < 15)
    {
        lds[offset_lds + ((thread / 5) * 25 + thread % 5 + 0) * lstride]
            = R[0]; /* stockham.py:577 */
        lds[offset_lds + ((thread / 5) * 25 + thread % 5 + 5) * lstride]
            = R[1]; /* stockham.py:577 */
        lds[offset_lds + ((thread / 5) * 25 + thread % 5 + 10) * lstride]
            = R[2]; /* stockham.py:577 */
        lds[offset_lds + ((thread / 5) * 25 + thread % 5 + 15) * lstride]
            = R[3]; /* stockham.py:577 */
        lds[offset_lds + ((thread / 5) * 25 + thread % 5 + 20) * lstride]
            = R[4]; /* stockham.py:577 */
    }

    // pass 2
    __syncthreads();
    R[0] = lds[offset_lds + (thread + 0) * lstride]; /* stockham.py:542 */
    R[1] = lds[offset_lds + (thread + 25) * lstride]; /* stockham.py:542 */
    R[2] = lds[offset_lds + (thread + 50) * lstride]; /* stockham.py:542 */

    W    = {twiddles[24 + 2 * (thread % 25)].x,
         -twiddles[24 + 2 * (thread % 25)].y}; /* generator.py:1065 */
    t.x  = W.x * R[1].x - W.y * R[1].y; /* stockham.py:555 */
    t.y  = W.y * R[1].x + W.x * R[1].y; /* stockham.py:556 */
    R[1] = t; /* stockham.py:557 */
    W    = {twiddles[25 + 2 * (thread % 25)].x,
         -twiddles[25 + 2 * (thread % 25)].y}; /* generator.py:1065 */
    t.x  = W.x * R[2].x - W.y * R[2].y; /* stockham.py:555 */
    t.y  = W.y * R[2].x + W.x * R[2].y; /* stockham.py:556 */
    R[2] = t; /* stockham.py:557 */

    InvRad3B1(&R[0], &R[1], &R[2]); /* stockham.py:566 */

    __syncthreads();
    if(write)
    {
        lds[offset_lds + ((thread / 25) * 75 + thread % 25 + 0) * lstride]
            = R[0]; /* stockham.py:577 */
        lds[offset_lds + ((thread / 25) * 75 + thread % 25 + 25) * lstride]
            = R[1]; /* stockham.py:577 */
        lds[offset_lds + ((thread / 25) * 75 + thread % 25 + 50) * lstride]
            = R[2]; /* stockham.py:577 */
    }
}
/* stockham.py:709 */
template <typename scalar_type, StrideBin sb, EmbeddedType ebtype, CallbackType cbtype>
__global__
    __launch_bounds__(250) void ip_inverse_length75_SBRR(const scalar_type* __restrict__ twiddles,
                                                         const size_t dim,
                                                         const size_t* __restrict__ lengths,
                                                         const size_t* __restrict__ stride,
                                                         const size_t       nbatch,
                                                         const unsigned int lds_padding,
                                                         void* __restrict__ load_cb_fn,
                                                         void* __restrict__ load_cb_data,
                                                         uint32_t load_cb_lds_bytes,
                                                         void* __restrict__ store_cb_fn,
                                                         void* __restrict__ store_cb_data,
                                                         scalar_type* __restrict__ buf)
{
    // this kernel:
    //   uses 25 threads per transform
    //   does 10 transforms per thread block
    // therefore it should be called with 250 threads per thread block
    extern __shared__ unsigned char __align__(sizeof(scalar_type)) lds_uchar[];
    scalar_type* __restrict__ lds = reinterpret_cast<scalar_type*>(lds_uchar);
    size_t       offset           = 0;
    unsigned int offset_lds;
    size_t       stride_lds;
    size_t       batch;
    size_t       transform;
    size_t       thread;
    bool         write;
    const size_t stride0  = (sb == SB_UNIT) ? (1) : (stride[0]);
    auto         load_cb  = get_load_cb<scalar_type, cbtype>(load_cb_fn);
    auto         store_cb = get_store_cb<scalar_type, cbtype>(store_cb_fn);

    // large twiddles

    // offsets
    size_t remaining;
    size_t plength = 1;
    size_t index_along_d;
    transform = blockIdx.x * 10 + threadIdx.x / 25; /* stockham.py:363 */
    remaining = transform; /* stockham.py:364 */
    for(int d = 1; d < dim; ++d)
    {
        plength       = plength * lengths[d]; /* stockham.py:367 */
        index_along_d = remaining % lengths[d]; /* stockham.py:368 */
        remaining     = remaining / lengths[d]; /* stockham.py:369 */
        offset        = offset + index_along_d * stride[d]; /* stockham.py:370 */
    }
    batch      = transform / plength; /* stockham.py:371 */
    offset     = offset + batch * stride[dim]; /* stockham.py:372 */
    offset_lds = (75 + lds_padding) * (transform % 10); /* stockham.py:373 */

    if(batch >= nbatch)
    {
        return;
    }

    // load global
    thread = threadIdx.x % 25; /* stockham.py:384 */
    lds[offset_lds + thread + 0]
        = load_cb(buf, offset + (thread + 0) * stride0, load_cb_data, nullptr);
    ; /* stockham.py:387 */
    lds[offset_lds + thread + 25]
        = load_cb(buf, offset + (thread + 25) * stride0, load_cb_data, nullptr);
    ; /* stockham.py:387 */
    lds[offset_lds + thread + 50]
        = load_cb(buf, offset + (thread + 50) * stride0, load_cb_data, nullptr);
    ; /* stockham.py:387 */

    // append extra global loading for C2Real pre-process only
    if(ebtype == EmbeddedType::C2Real_PRE)
    {
        // use the last thread of each transform to load one more element per row
        if(thread == 24)
        {
            lds[offset_lds + thread + 50 + 1]
                = load_cb(buf, offset + (thread + 50 + 1) * stride0, load_cb_data, nullptr);
            ; /* stockham.py:395 */
        }
    }

    // handle even-length real to complex pre-process in lds before transform
    if(ebtype == EmbeddedType::C2Real_PRE)
    {
        __syncthreads();

        real_pre_process_kernel_inplace<scalar_type, false>(thread % 38 + 0,
                                                            75 - thread % 38 - 0,
                                                            38,
                                                            &lds[offset_lds],
                                                            0,
                                                            &twiddles[75]); /* stockham.py:338 */

        real_pre_process_kernel_inplace<scalar_type, false>(thread % 38 + 25,
                                                            75 - thread % 38 - 25,
                                                            38,
                                                            &lds[offset_lds],
                                                            0,
                                                            &twiddles[75]); /* stockham.py:338 */

        __syncthreads();
    }

    // transform
    write = true; /* stockham.py:681 */
    ip_inverse_length75_SBRR_device<scalar_type, SB_UNIT>(
        lds, twiddles, stride_lds, offset_lds, write); /* stockham.py:686 */

    // handle even-length complex to real post-process in lds after transform
    if(ebtype == EmbeddedType::Real2C_POST)
    {
        __syncthreads();

        real_post_process_kernel_inplace<scalar_type, false>(thread % 38 + 0,
                                                             75 - thread % 38 - 0,
                                                             38,
                                                             &lds[offset_lds],
                                                             0,
                                                             &twiddles[75]); /* stockham.py:338 */

        real_post_process_kernel_inplace<scalar_type, false>(thread % 38 + 25,
                                                             75 - thread % 38 - 25,
                                                             38,
                                                             &lds[offset_lds],
                                                             0,
                                                             &twiddles[75]); /* stockham.py:338 */
    }

    // store global
    __syncthreads();
    store_cb(
        buf, offset + (thread + 0) * stride0, lds[offset_lds + thread + 0], store_cb_data, nullptr);
    store_cb(buf,
             offset + (thread + 25) * stride0,
             lds[offset_lds + thread + 25],
             store_cb_data,
             nullptr);
    store_cb(buf,
             offset + (thread + 50) * stride0,
             lds[offset_lds + thread + 50],
             store_cb_data,
             nullptr);

    // append extra global write for Real2C post-process only
    if(ebtype == EmbeddedType::Real2C_POST)
    {
        // use the last thread of each transform to write one more element per row
        if(thread == 24)
        {
            store_cb(buf,
                     offset + (thread + 50 + 1) * stride0,
                     lds[offset_lds + thread + 50 + 1],
                     store_cb_data,
                     nullptr);
        }
    }
}
/* stockham.py:709 */
template <typename scalar_type, StrideBin sb, EmbeddedType ebtype, CallbackType cbtype>
__global__ __launch_bounds__(250) void ip_inverse_length75_SBRR(
    const scalar_type* __restrict__ twiddles,
    const size_t dim,
    const size_t* __restrict__ lengths,
    const size_t* __restrict__ stride,
    const size_t       nbatch,
    const unsigned int lds_padding,
    void* __restrict__ load_cb_fn,
    void* __restrict__ load_cb_data,
    uint32_t load_cb_lds_bytes,
    void* __restrict__ store_cb_fn,
    void* __restrict__ store_cb_data,
    real_type_t<scalar_type>* __restrict__ bufre,
    real_type_t<scalar_type>* __restrict__ bufim)
{
    // this kernel:
    //   uses 25 threads per transform
    //   does 10 transforms per thread block
    // therefore it should be called with 250 threads per thread block
    extern __shared__ unsigned char __align__(sizeof(scalar_type)) lds_uchar[];
    scalar_type* __restrict__ lds = reinterpret_cast<scalar_type*>(lds_uchar);
    size_t       offset           = 0;
    unsigned int offset_lds;
    size_t       stride_lds;
    size_t       batch;
    size_t       transform;
    size_t       thread;
    bool         write;
    const size_t stride0  = (sb == SB_UNIT) ? (1) : (stride[0]);
    auto         load_cb  = get_load_cb<scalar_type, cbtype>(load_cb_fn);
    auto         store_cb = get_store_cb<scalar_type, cbtype>(store_cb_fn);

    // large twiddles

    // offsets
    size_t remaining;
    size_t plength = 1;
    size_t index_along_d;
    transform = blockIdx.x * 10 + threadIdx.x / 25; /* stockham.py:363 */
    remaining = transform; /* stockham.py:364 */
    for(int d = 1; d < dim; ++d)
    {
        plength       = plength * lengths[d]; /* stockham.py:367 */
        index_along_d = remaining % lengths[d]; /* stockham.py:368 */
        remaining     = remaining / lengths[d]; /* stockham.py:369 */
        offset        = offset + index_along_d * stride[d]; /* stockham.py:370 */
    }
    batch      = transform / plength; /* stockham.py:371 */
    offset     = offset + batch * stride[dim]; /* stockham.py:372 */
    offset_lds = (75 + lds_padding) * (transform % 10); /* stockham.py:373 */

    if(batch >= nbatch)
    {
        return;
    }

    // load global
    thread                       = threadIdx.x % 25; /* stockham.py:384 */
    lds[offset_lds + thread + 0] = {bufre[offset + (thread + 0) * stride0],
                                    bufim[offset + (thread + 0) * stride0]}; /* generator.py:872 */
    lds[offset_lds + thread + 25]
        = {bufre[offset + (thread + 25) * stride0],
           bufim[offset + (thread + 25) * stride0]}; /* generator.py:872 */
    lds[offset_lds + thread + 50]
        = {bufre[offset + (thread + 50) * stride0],
           bufim[offset + (thread + 50) * stride0]}; /* generator.py:872 */

    // append extra global loading for C2Real pre-process only
    if(ebtype == EmbeddedType::C2Real_PRE)
    {
        // use the last thread of each transform to load one more element per row
        if(thread == 24)
        {
            lds[offset_lds + thread + 50 + 1]
                = {bufre[offset + (thread + 50 + 1) * stride0],
                   bufim[offset + (thread + 50 + 1) * stride0]}; /* generator.py:872 */
        }
    }

    // handle even-length real to complex pre-process in lds before transform
    if(ebtype == EmbeddedType::C2Real_PRE)
    {
        __syncthreads();

        real_pre_process_kernel_inplace<scalar_type, false>(thread % 38 + 0,
                                                            75 - thread % 38 - 0,
                                                            38,
                                                            &lds[offset_lds],
                                                            0,
                                                            &twiddles[75]); /* stockham.py:338 */

        real_pre_process_kernel_inplace<scalar_type, false>(thread % 38 + 25,
                                                            75 - thread % 38 - 25,
                                                            38,
                                                            &lds[offset_lds],
                                                            0,
                                                            &twiddles[75]); /* stockham.py:338 */

        __syncthreads();
    }

    // transform
    write = true; /* stockham.py:681 */
    ip_inverse_length75_SBRR_device<scalar_type, SB_UNIT>(
        lds, twiddles, stride_lds, offset_lds, write); /* stockham.py:686 */

    // handle even-length complex to real post-process in lds after transform
    if(ebtype == EmbeddedType::Real2C_POST)
    {
        __syncthreads();

        real_post_process_kernel_inplace<scalar_type, false>(thread % 38 + 0,
                                                             75 - thread % 38 - 0,
                                                             38,
                                                             &lds[offset_lds],
                                                             0,
                                                             &twiddles[75]); /* stockham.py:338 */

        real_post_process_kernel_inplace<scalar_type, false>(thread % 38 + 25,
                                                             75 - thread % 38 - 25,
                                                             38,
                                                             &lds[offset_lds],
                                                             0,
                                                             &twiddles[75]); /* stockham.py:338 */
    }

    // store global
    __syncthreads();
    bufre[offset + (thread + 0) * stride0] = lds[offset_lds + thread + 0].x; /* generator.py:906 */
    bufim[offset + (thread + 0) * stride0] = lds[offset_lds + thread + 0].y; /* generator.py:908 */
    bufre[offset + (thread + 25) * stride0]
        = lds[offset_lds + thread + 25].x; /* generator.py:906 */
    bufim[offset + (thread + 25) * stride0]
        = lds[offset_lds + thread + 25].y; /* generator.py:908 */
    bufre[offset + (thread + 50) * stride0]
        = lds[offset_lds + thread + 50].x; /* generator.py:906 */
    bufim[offset + (thread + 50) * stride0]
        = lds[offset_lds + thread + 50].y; /* generator.py:908 */

    // append extra global write for Real2C post-process only
    if(ebtype == EmbeddedType::Real2C_POST)
    {
        // use the last thread of each transform to write one more element per row
        if(thread == 24)
        {
            bufre[offset + (thread + 50 + 1) * stride0]
                = lds[offset_lds + thread + 50 + 1].x; /* generator.py:906 */
            bufim[offset + (thread + 50 + 1) * stride0]
                = lds[offset_lds + thread + 50 + 1].y; /* generator.py:908 */
        }
    }
}
/* stockham.py:765 */
template <typename scalar_type, StrideBin sb>
__device__ void op_inverse_length75_SBRR_device(scalar_type* __restrict__ lds,
                                                const scalar_type* __restrict__ twiddles,
                                                size_t       stride_lds,
                                                unsigned int offset_lds,
                                                bool         write)
{
    size_t       thread;
    scalar_type  R[5];
    scalar_type  W;
    scalar_type  t;
    const size_t lstride = (sb == SB_UNIT) ? (1) : (stride_lds);
    thread               = threadIdx.x % 25; /* stockham.py:731 */

    // pass 0
    __syncthreads();
    R[0] = lds[offset_lds + (thread + 0) * lstride]; /* stockham.py:542 */
    R[1] = lds[offset_lds + (thread + 15) * lstride]; /* stockham.py:542 */
    R[2] = lds[offset_lds + (thread + 30) * lstride]; /* stockham.py:542 */
    R[3] = lds[offset_lds + (thread + 45) * lstride]; /* stockham.py:542 */
    R[4] = lds[offset_lds + (thread + 60) * lstride]; /* stockham.py:542 */

    InvRad5B1(&R[0], &R[1], &R[2], &R[3], &R[4]); /* stockham.py:566 */

    __syncthreads();
    if(write && thread < 15)
    {
        lds[offset_lds + ((thread / 1) * 5 + thread % 1 + 0) * lstride]
            = R[0]; /* stockham.py:577 */
        lds[offset_lds + ((thread / 1) * 5 + thread % 1 + 1) * lstride]
            = R[1]; /* stockham.py:577 */
        lds[offset_lds + ((thread / 1) * 5 + thread % 1 + 2) * lstride]
            = R[2]; /* stockham.py:577 */
        lds[offset_lds + ((thread / 1) * 5 + thread % 1 + 3) * lstride]
            = R[3]; /* stockham.py:577 */
        lds[offset_lds + ((thread / 1) * 5 + thread % 1 + 4) * lstride]
            = R[4]; /* stockham.py:577 */
    }

    // pass 1
    __syncthreads();
    R[0] = lds[offset_lds + (thread + 0) * lstride]; /* stockham.py:542 */
    R[1] = lds[offset_lds + (thread + 15) * lstride]; /* stockham.py:542 */
    R[2] = lds[offset_lds + (thread + 30) * lstride]; /* stockham.py:542 */
    R[3] = lds[offset_lds + (thread + 45) * lstride]; /* stockham.py:542 */
    R[4] = lds[offset_lds + (thread + 60) * lstride]; /* stockham.py:542 */

    W    = {twiddles[4 + 4 * (thread % 5)].x,
         -twiddles[4 + 4 * (thread % 5)].y}; /* generator.py:1065 */
    t.x  = W.x * R[1].x - W.y * R[1].y; /* stockham.py:555 */
    t.y  = W.y * R[1].x + W.x * R[1].y; /* stockham.py:556 */
    R[1] = t; /* stockham.py:557 */
    W    = {twiddles[5 + 4 * (thread % 5)].x,
         -twiddles[5 + 4 * (thread % 5)].y}; /* generator.py:1065 */
    t.x  = W.x * R[2].x - W.y * R[2].y; /* stockham.py:555 */
    t.y  = W.y * R[2].x + W.x * R[2].y; /* stockham.py:556 */
    R[2] = t; /* stockham.py:557 */
    W    = {twiddles[6 + 4 * (thread % 5)].x,
         -twiddles[6 + 4 * (thread % 5)].y}; /* generator.py:1065 */
    t.x  = W.x * R[3].x - W.y * R[3].y; /* stockham.py:555 */
    t.y  = W.y * R[3].x + W.x * R[3].y; /* stockham.py:556 */
    R[3] = t; /* stockham.py:557 */
    W    = {twiddles[7 + 4 * (thread % 5)].x,
         -twiddles[7 + 4 * (thread % 5)].y}; /* generator.py:1065 */
    t.x  = W.x * R[4].x - W.y * R[4].y; /* stockham.py:555 */
    t.y  = W.y * R[4].x + W.x * R[4].y; /* stockham.py:556 */
    R[4] = t; /* stockham.py:557 */

    InvRad5B1(&R[0], &R[1], &R[2], &R[3], &R[4]); /* stockham.py:566 */

    __syncthreads();
    if(write && thread < 15)
    {
        lds[offset_lds + ((thread / 5) * 25 + thread % 5 + 0) * lstride]
            = R[0]; /* stockham.py:577 */
        lds[offset_lds + ((thread / 5) * 25 + thread % 5 + 5) * lstride]
            = R[1]; /* stockham.py:577 */
        lds[offset_lds + ((thread / 5) * 25 + thread % 5 + 10) * lstride]
            = R[2]; /* stockham.py:577 */
        lds[offset_lds + ((thread / 5) * 25 + thread % 5 + 15) * lstride]
            = R[3]; /* stockham.py:577 */
        lds[offset_lds + ((thread / 5) * 25 + thread % 5 + 20) * lstride]
            = R[4]; /* stockham.py:577 */
    }

    // pass 2
    __syncthreads();
    R[0] = lds[offset_lds + (thread + 0) * lstride]; /* stockham.py:542 */
    R[1] = lds[offset_lds + (thread + 25) * lstride]; /* stockham.py:542 */
    R[2] = lds[offset_lds + (thread + 50) * lstride]; /* stockham.py:542 */

    W    = {twiddles[24 + 2 * (thread % 25)].x,
         -twiddles[24 + 2 * (thread % 25)].y}; /* generator.py:1065 */
    t.x  = W.x * R[1].x - W.y * R[1].y; /* stockham.py:555 */
    t.y  = W.y * R[1].x + W.x * R[1].y; /* stockham.py:556 */
    R[1] = t; /* stockham.py:557 */
    W    = {twiddles[25 + 2 * (thread % 25)].x,
         -twiddles[25 + 2 * (thread % 25)].y}; /* generator.py:1065 */
    t.x  = W.x * R[2].x - W.y * R[2].y; /* stockham.py:555 */
    t.y  = W.y * R[2].x + W.x * R[2].y; /* stockham.py:556 */
    R[2] = t; /* stockham.py:557 */

    InvRad3B1(&R[0], &R[1], &R[2]); /* stockham.py:566 */

    __syncthreads();
    if(write)
    {
        lds[offset_lds + ((thread / 25) * 75 + thread % 25 + 0) * lstride]
            = R[0]; /* stockham.py:577 */
        lds[offset_lds + ((thread / 25) * 75 + thread % 25 + 25) * lstride]
            = R[1]; /* stockham.py:577 */
        lds[offset_lds + ((thread / 25) * 75 + thread % 25 + 50) * lstride]
            = R[2]; /* stockham.py:577 */
    }
}
/* stockham.py:709 */
template <typename scalar_type, StrideBin sb, EmbeddedType ebtype, CallbackType cbtype>
__global__
    __launch_bounds__(250) void op_inverse_length75_SBRR(const scalar_type* __restrict__ twiddles,
                                                         const size_t dim,
                                                         const size_t* __restrict__ lengths,
                                                         const size_t* __restrict__ stride_in,
                                                         const size_t* __restrict__ stride_out,
                                                         const size_t       nbatch,
                                                         const unsigned int lds_padding,
                                                         void* __restrict__ load_cb_fn,
                                                         void* __restrict__ load_cb_data,
                                                         uint32_t load_cb_lds_bytes,
                                                         void* __restrict__ store_cb_fn,
                                                         void* __restrict__ store_cb_data,
                                                         scalar_type* __restrict__ buf_in,
                                                         scalar_type* __restrict__ buf_out)
{
    // this kernel:
    //   uses 25 threads per transform
    //   does 10 transforms per thread block
    // therefore it should be called with 250 threads per thread block
    extern __shared__ unsigned char __align__(sizeof(scalar_type)) lds_uchar[];
    scalar_type* __restrict__ lds = reinterpret_cast<scalar_type*>(lds_uchar);
    size_t       offset_in        = 0;
    size_t       offset_out       = 0;
    unsigned int offset_lds;
    size_t       stride_lds;
    size_t       batch;
    size_t       transform;
    size_t       thread;
    bool         write;
    const size_t stride0_in  = (sb == SB_UNIT) ? (1) : (stride_in[0]);
    const size_t stride0_out = (sb == SB_UNIT) ? (1) : (stride_out[0]);
    auto         load_cb     = get_load_cb<scalar_type, cbtype>(load_cb_fn);
    auto         store_cb    = get_store_cb<scalar_type, cbtype>(store_cb_fn);

    // large twiddles

    // offsets
    size_t remaining;
    size_t plength = 1;
    size_t index_along_d;
    transform = blockIdx.x * 10 + threadIdx.x / 25; /* stockham.py:363 */
    remaining = transform; /* stockham.py:364 */
    for(int d = 1; d < dim; ++d)
    {
        plength       = plength * lengths[d]; /* stockham.py:367 */
        index_along_d = remaining % lengths[d]; /* stockham.py:368 */
        remaining     = remaining / lengths[d]; /* stockham.py:369 */
        offset_in     = offset_in + index_along_d * stride_in[d]; /* generator.py:992 */
        offset_out    = offset_out + index_along_d * stride_out[d]; /* generator.py:993 */
    }
    batch      = transform / plength; /* stockham.py:371 */
    offset_in  = offset_in + batch * stride_in[dim]; /* generator.py:992 */
    offset_out = offset_out + batch * stride_out[dim]; /* generator.py:993 */
    offset_lds = (75 + lds_padding) * (transform % 10); /* stockham.py:373 */

    if(batch >= nbatch)
    {
        return;
    }

    // load global
    thread = threadIdx.x % 25; /* stockham.py:384 */
    lds[offset_lds + thread + 0]
        = load_cb(buf_in, offset_in + (thread + 0) * stride0_in, load_cb_data, nullptr);
    ; /* stockham.py:387 */
    lds[offset_lds + thread + 25]
        = load_cb(buf_in, offset_in + (thread + 25) * stride0_in, load_cb_data, nullptr);
    ; /* stockham.py:387 */
    lds[offset_lds + thread + 50]
        = load_cb(buf_in, offset_in + (thread + 50) * stride0_in, load_cb_data, nullptr);
    ; /* stockham.py:387 */

    // append extra global loading for C2Real pre-process only
    if(ebtype == EmbeddedType::C2Real_PRE)
    {
        // use the last thread of each transform to load one more element per row
        if(thread == 24)
        {
            lds[offset_lds + thread + 50 + 1] = load_cb(
                buf_in, offset_in + (thread + 50 + 1) * stride0_in, load_cb_data, nullptr);
            ; /* stockham.py:395 */
        }
    }

    // handle even-length real to complex pre-process in lds before transform
    if(ebtype == EmbeddedType::C2Real_PRE)
    {
        __syncthreads();

        real_pre_process_kernel_inplace<scalar_type, false>(thread % 38 + 0,
                                                            75 - thread % 38 - 0,
                                                            38,
                                                            &lds[offset_lds],
                                                            0,
                                                            &twiddles[75]); /* stockham.py:338 */

        real_pre_process_kernel_inplace<scalar_type, false>(thread % 38 + 25,
                                                            75 - thread % 38 - 25,
                                                            38,
                                                            &lds[offset_lds],
                                                            0,
                                                            &twiddles[75]); /* stockham.py:338 */

        __syncthreads();
    }

    // transform
    write = true; /* stockham.py:681 */
    op_inverse_length75_SBRR_device<scalar_type, SB_UNIT>(
        lds, twiddles, stride_lds, offset_lds, write); /* stockham.py:686 */

    // handle even-length complex to real post-process in lds after transform
    if(ebtype == EmbeddedType::Real2C_POST)
    {
        __syncthreads();

        real_post_process_kernel_inplace<scalar_type, false>(thread % 38 + 0,
                                                             75 - thread % 38 - 0,
                                                             38,
                                                             &lds[offset_lds],
                                                             0,
                                                             &twiddles[75]); /* stockham.py:338 */

        real_post_process_kernel_inplace<scalar_type, false>(thread % 38 + 25,
                                                             75 - thread % 38 - 25,
                                                             38,
                                                             &lds[offset_lds],
                                                             0,
                                                             &twiddles[75]); /* stockham.py:338 */
    }

    // store global
    __syncthreads();
    store_cb(buf_out,
             offset_out + (thread + 0) * stride0_out,
             lds[offset_lds + thread + 0],
             store_cb_data,
             nullptr);
    store_cb(buf_out,
             offset_out + (thread + 25) * stride0_out,
             lds[offset_lds + thread + 25],
             store_cb_data,
             nullptr);
    store_cb(buf_out,
             offset_out + (thread + 50) * stride0_out,
             lds[offset_lds + thread + 50],
             store_cb_data,
             nullptr);

    // append extra global write for Real2C post-process only
    if(ebtype == EmbeddedType::Real2C_POST)
    {
        // use the last thread of each transform to write one more element per row
        if(thread == 24)
        {
            store_cb(buf_out,
                     offset_out + (thread + 50 + 1) * stride0_out,
                     lds[offset_lds + thread + 50 + 1],
                     store_cb_data,
                     nullptr);
        }
    }
}
/* stockham.py:709 */
template <typename scalar_type, StrideBin sb, EmbeddedType ebtype, CallbackType cbtype>
__global__ __launch_bounds__(250) void op_inverse_length75_SBRR(
    const scalar_type* __restrict__ twiddles,
    const size_t dim,
    const size_t* __restrict__ lengths,
    const size_t* __restrict__ stride_in,
    const size_t* __restrict__ stride_out,
    const size_t       nbatch,
    const unsigned int lds_padding,
    void* __restrict__ load_cb_fn,
    void* __restrict__ load_cb_data,
    uint32_t load_cb_lds_bytes,
    void* __restrict__ store_cb_fn,
    void* __restrict__ store_cb_data,
    scalar_type* __restrict__ buf_in,
    real_type_t<scalar_type>* __restrict__ buf_outre,
    real_type_t<scalar_type>* __restrict__ buf_outim)
{
    // this kernel:
    //   uses 25 threads per transform
    //   does 10 transforms per thread block
    // therefore it should be called with 250 threads per thread block
    extern __shared__ unsigned char __align__(sizeof(scalar_type)) lds_uchar[];
    scalar_type* __restrict__ lds = reinterpret_cast<scalar_type*>(lds_uchar);
    size_t       offset_in        = 0;
    size_t       offset_out       = 0;
    unsigned int offset_lds;
    size_t       stride_lds;
    size_t       batch;
    size_t       transform;
    size_t       thread;
    bool         write;
    const size_t stride0_in  = (sb == SB_UNIT) ? (1) : (stride_in[0]);
    const size_t stride0_out = (sb == SB_UNIT) ? (1) : (stride_out[0]);
    auto         load_cb     = get_load_cb<scalar_type, cbtype>(load_cb_fn);
    auto         store_cb    = get_store_cb<scalar_type, cbtype>(store_cb_fn);

    // large twiddles

    // offsets
    size_t remaining;
    size_t plength = 1;
    size_t index_along_d;
    transform = blockIdx.x * 10 + threadIdx.x / 25; /* stockham.py:363 */
    remaining = transform; /* stockham.py:364 */
    for(int d = 1; d < dim; ++d)
    {
        plength       = plength * lengths[d]; /* stockham.py:367 */
        index_along_d = remaining % lengths[d]; /* stockham.py:368 */
        remaining     = remaining / lengths[d]; /* stockham.py:369 */
        offset_in     = offset_in + index_along_d * stride_in[d]; /* generator.py:992 */
        offset_out    = offset_out + index_along_d * stride_out[d]; /* generator.py:993 */
    }
    batch      = transform / plength; /* stockham.py:371 */
    offset_in  = offset_in + batch * stride_in[dim]; /* generator.py:992 */
    offset_out = offset_out + batch * stride_out[dim]; /* generator.py:993 */
    offset_lds = (75 + lds_padding) * (transform % 10); /* stockham.py:373 */

    if(batch >= nbatch)
    {
        return;
    }

    // load global
    thread = threadIdx.x % 25; /* stockham.py:384 */
    lds[offset_lds + thread + 0]
        = load_cb(buf_in, offset_in + (thread + 0) * stride0_in, load_cb_data, nullptr);
    ; /* stockham.py:387 */
    lds[offset_lds + thread + 25]
        = load_cb(buf_in, offset_in + (thread + 25) * stride0_in, load_cb_data, nullptr);
    ; /* stockham.py:387 */
    lds[offset_lds + thread + 50]
        = load_cb(buf_in, offset_in + (thread + 50) * stride0_in, load_cb_data, nullptr);
    ; /* stockham.py:387 */

    // append extra global loading for C2Real pre-process only
    if(ebtype == EmbeddedType::C2Real_PRE)
    {
        // use the last thread of each transform to load one more element per row
        if(thread == 24)
        {
            lds[offset_lds + thread + 50 + 1] = load_cb(
                buf_in, offset_in + (thread + 50 + 1) * stride0_in, load_cb_data, nullptr);
            ; /* stockham.py:395 */
        }
    }

    // handle even-length real to complex pre-process in lds before transform
    if(ebtype == EmbeddedType::C2Real_PRE)
    {
        __syncthreads();

        real_pre_process_kernel_inplace<scalar_type, false>(thread % 38 + 0,
                                                            75 - thread % 38 - 0,
                                                            38,
                                                            &lds[offset_lds],
                                                            0,
                                                            &twiddles[75]); /* stockham.py:338 */

        real_pre_process_kernel_inplace<scalar_type, false>(thread % 38 + 25,
                                                            75 - thread % 38 - 25,
                                                            38,
                                                            &lds[offset_lds],
                                                            0,
                                                            &twiddles[75]); /* stockham.py:338 */

        __syncthreads();
    }

    // transform
    write = true; /* stockham.py:681 */
    op_inverse_length75_SBRR_device<scalar_type, SB_UNIT>(
        lds, twiddles, stride_lds, offset_lds, write); /* stockham.py:686 */

    // handle even-length complex to real post-process in lds after transform
    if(ebtype == EmbeddedType::Real2C_POST)
    {
        __syncthreads();

        real_post_process_kernel_inplace<scalar_type, false>(thread % 38 + 0,
                                                             75 - thread % 38 - 0,
                                                             38,
                                                             &lds[offset_lds],
                                                             0,
                                                             &twiddles[75]); /* stockham.py:338 */

        real_post_process_kernel_inplace<scalar_type, false>(thread % 38 + 25,
                                                             75 - thread % 38 - 25,
                                                             38,
                                                             &lds[offset_lds],
                                                             0,
                                                             &twiddles[75]); /* stockham.py:338 */
    }

    // store global
    __syncthreads();
    buf_outre[offset_out + (thread + 0) * stride0_out]
        = lds[offset_lds + thread + 0].x; /* generator.py:906 */
    buf_outim[offset_out + (thread + 0) * stride0_out]
        = lds[offset_lds + thread + 0].y; /* generator.py:908 */
    buf_outre[offset_out + (thread + 25) * stride0_out]
        = lds[offset_lds + thread + 25].x; /* generator.py:906 */
    buf_outim[offset_out + (thread + 25) * stride0_out]
        = lds[offset_lds + thread + 25].y; /* generator.py:908 */
    buf_outre[offset_out + (thread + 50) * stride0_out]
        = lds[offset_lds + thread + 50].x; /* generator.py:906 */
    buf_outim[offset_out + (thread + 50) * stride0_out]
        = lds[offset_lds + thread + 50].y; /* generator.py:908 */

    // append extra global write for Real2C post-process only
    if(ebtype == EmbeddedType::Real2C_POST)
    {
        // use the last thread of each transform to write one more element per row
        if(thread == 24)
        {
            buf_outre[offset_out + (thread + 50 + 1) * stride0_out]
                = lds[offset_lds + thread + 50 + 1].x; /* generator.py:906 */
            buf_outim[offset_out + (thread + 50 + 1) * stride0_out]
                = lds[offset_lds + thread + 50 + 1].y; /* generator.py:908 */
        }
    }
}
/* stockham.py:709 */
template <typename scalar_type, StrideBin sb, EmbeddedType ebtype, CallbackType cbtype>
__global__ __launch_bounds__(250) void op_inverse_length75_SBRR(
    const scalar_type* __restrict__ twiddles,
    const size_t dim,
    const size_t* __restrict__ lengths,
    const size_t* __restrict__ stride_in,
    const size_t* __restrict__ stride_out,
    const size_t       nbatch,
    const unsigned int lds_padding,
    void* __restrict__ load_cb_fn,
    void* __restrict__ load_cb_data,
    uint32_t load_cb_lds_bytes,
    void* __restrict__ store_cb_fn,
    void* __restrict__ store_cb_data,
    real_type_t<scalar_type>* __restrict__ buf_inre,
    real_type_t<scalar_type>* __restrict__ buf_inim,
    scalar_type* __restrict__ buf_out)
{
    // this kernel:
    //   uses 25 threads per transform
    //   does 10 transforms per thread block
    // therefore it should be called with 250 threads per thread block
    extern __shared__ unsigned char __align__(sizeof(scalar_type)) lds_uchar[];
    scalar_type* __restrict__ lds = reinterpret_cast<scalar_type*>(lds_uchar);
    size_t       offset_in        = 0;
    size_t       offset_out       = 0;
    unsigned int offset_lds;
    size_t       stride_lds;
    size_t       batch;
    size_t       transform;
    size_t       thread;
    bool         write;
    const size_t stride0_in  = (sb == SB_UNIT) ? (1) : (stride_in[0]);
    const size_t stride0_out = (sb == SB_UNIT) ? (1) : (stride_out[0]);
    auto         load_cb     = get_load_cb<scalar_type, cbtype>(load_cb_fn);
    auto         store_cb    = get_store_cb<scalar_type, cbtype>(store_cb_fn);

    // large twiddles

    // offsets
    size_t remaining;
    size_t plength = 1;
    size_t index_along_d;
    transform = blockIdx.x * 10 + threadIdx.x / 25; /* stockham.py:363 */
    remaining = transform; /* stockham.py:364 */
    for(int d = 1; d < dim; ++d)
    {
        plength       = plength * lengths[d]; /* stockham.py:367 */
        index_along_d = remaining % lengths[d]; /* stockham.py:368 */
        remaining     = remaining / lengths[d]; /* stockham.py:369 */
        offset_in     = offset_in + index_along_d * stride_in[d]; /* generator.py:992 */
        offset_out    = offset_out + index_along_d * stride_out[d]; /* generator.py:993 */
    }
    batch      = transform / plength; /* stockham.py:371 */
    offset_in  = offset_in + batch * stride_in[dim]; /* generator.py:992 */
    offset_out = offset_out + batch * stride_out[dim]; /* generator.py:993 */
    offset_lds = (75 + lds_padding) * (transform % 10); /* stockham.py:373 */

    if(batch >= nbatch)
    {
        return;
    }

    // load global
    thread = threadIdx.x % 25; /* stockham.py:384 */
    lds[offset_lds + thread + 0]
        = {buf_inre[offset_in + (thread + 0) * stride0_in],
           buf_inim[offset_in + (thread + 0) * stride0_in]}; /* generator.py:872 */
    lds[offset_lds + thread + 25]
        = {buf_inre[offset_in + (thread + 25) * stride0_in],
           buf_inim[offset_in + (thread + 25) * stride0_in]}; /* generator.py:872 */
    lds[offset_lds + thread + 50]
        = {buf_inre[offset_in + (thread + 50) * stride0_in],
           buf_inim[offset_in + (thread + 50) * stride0_in]}; /* generator.py:872 */

    // append extra global loading for C2Real pre-process only
    if(ebtype == EmbeddedType::C2Real_PRE)
    {
        // use the last thread of each transform to load one more element per row
        if(thread == 24)
        {
            lds[offset_lds + thread + 50 + 1]
                = {buf_inre[offset_in + (thread + 50 + 1) * stride0_in],
                   buf_inim[offset_in + (thread + 50 + 1) * stride0_in]}; /* generator.py:872 */
        }
    }

    // handle even-length real to complex pre-process in lds before transform
    if(ebtype == EmbeddedType::C2Real_PRE)
    {
        __syncthreads();

        real_pre_process_kernel_inplace<scalar_type, false>(thread % 38 + 0,
                                                            75 - thread % 38 - 0,
                                                            38,
                                                            &lds[offset_lds],
                                                            0,
                                                            &twiddles[75]); /* stockham.py:338 */

        real_pre_process_kernel_inplace<scalar_type, false>(thread % 38 + 25,
                                                            75 - thread % 38 - 25,
                                                            38,
                                                            &lds[offset_lds],
                                                            0,
                                                            &twiddles[75]); /* stockham.py:338 */

        __syncthreads();
    }

    // transform
    write = true; /* stockham.py:681 */
    op_inverse_length75_SBRR_device<scalar_type, SB_UNIT>(
        lds, twiddles, stride_lds, offset_lds, write); /* stockham.py:686 */

    // handle even-length complex to real post-process in lds after transform
    if(ebtype == EmbeddedType::Real2C_POST)
    {
        __syncthreads();

        real_post_process_kernel_inplace<scalar_type, false>(thread % 38 + 0,
                                                             75 - thread % 38 - 0,
                                                             38,
                                                             &lds[offset_lds],
                                                             0,
                                                             &twiddles[75]); /* stockham.py:338 */

        real_post_process_kernel_inplace<scalar_type, false>(thread % 38 + 25,
                                                             75 - thread % 38 - 25,
                                                             38,
                                                             &lds[offset_lds],
                                                             0,
                                                             &twiddles[75]); /* stockham.py:338 */
    }

    // store global
    __syncthreads();
    store_cb(buf_out,
             offset_out + (thread + 0) * stride0_out,
             lds[offset_lds + thread + 0],
             store_cb_data,
             nullptr);
    store_cb(buf_out,
             offset_out + (thread + 25) * stride0_out,
             lds[offset_lds + thread + 25],
             store_cb_data,
             nullptr);
    store_cb(buf_out,
             offset_out + (thread + 50) * stride0_out,
             lds[offset_lds + thread + 50],
             store_cb_data,
             nullptr);

    // append extra global write for Real2C post-process only
    if(ebtype == EmbeddedType::Real2C_POST)
    {
        // use the last thread of each transform to write one more element per row
        if(thread == 24)
        {
            store_cb(buf_out,
                     offset_out + (thread + 50 + 1) * stride0_out,
                     lds[offset_lds + thread + 50 + 1],
                     store_cb_data,
                     nullptr);
        }
    }
}
/* stockham.py:709 */
template <typename scalar_type, StrideBin sb, EmbeddedType ebtype, CallbackType cbtype>
__global__ __launch_bounds__(250) void op_inverse_length75_SBRR(
    const scalar_type* __restrict__ twiddles,
    const size_t dim,
    const size_t* __restrict__ lengths,
    const size_t* __restrict__ stride_in,
    const size_t* __restrict__ stride_out,
    const size_t       nbatch,
    const unsigned int lds_padding,
    void* __restrict__ load_cb_fn,
    void* __restrict__ load_cb_data,
    uint32_t load_cb_lds_bytes,
    void* __restrict__ store_cb_fn,
    void* __restrict__ store_cb_data,
    real_type_t<scalar_type>* __restrict__ buf_inre,
    real_type_t<scalar_type>* __restrict__ buf_inim,
    real_type_t<scalar_type>* __restrict__ buf_outre,
    real_type_t<scalar_type>* __restrict__ buf_outim)
{
    // this kernel:
    //   uses 25 threads per transform
    //   does 10 transforms per thread block
    // therefore it should be called with 250 threads per thread block
    extern __shared__ unsigned char __align__(sizeof(scalar_type)) lds_uchar[];
    scalar_type* __restrict__ lds = reinterpret_cast<scalar_type*>(lds_uchar);
    size_t       offset_in        = 0;
    size_t       offset_out       = 0;
    unsigned int offset_lds;
    size_t       stride_lds;
    size_t       batch;
    size_t       transform;
    size_t       thread;
    bool         write;
    const size_t stride0_in  = (sb == SB_UNIT) ? (1) : (stride_in[0]);
    const size_t stride0_out = (sb == SB_UNIT) ? (1) : (stride_out[0]);
    auto         load_cb     = get_load_cb<scalar_type, cbtype>(load_cb_fn);
    auto         store_cb    = get_store_cb<scalar_type, cbtype>(store_cb_fn);

    // large twiddles

    // offsets
    size_t remaining;
    size_t plength = 1;
    size_t index_along_d;
    transform = blockIdx.x * 10 + threadIdx.x / 25; /* stockham.py:363 */
    remaining = transform; /* stockham.py:364 */
    for(int d = 1; d < dim; ++d)
    {
        plength       = plength * lengths[d]; /* stockham.py:367 */
        index_along_d = remaining % lengths[d]; /* stockham.py:368 */
        remaining     = remaining / lengths[d]; /* stockham.py:369 */
        offset_in     = offset_in + index_along_d * stride_in[d]; /* generator.py:992 */
        offset_out    = offset_out + index_along_d * stride_out[d]; /* generator.py:993 */
    }
    batch      = transform / plength; /* stockham.py:371 */
    offset_in  = offset_in + batch * stride_in[dim]; /* generator.py:992 */
    offset_out = offset_out + batch * stride_out[dim]; /* generator.py:993 */
    offset_lds = (75 + lds_padding) * (transform % 10); /* stockham.py:373 */

    if(batch >= nbatch)
    {
        return;
    }

    // load global
    thread = threadIdx.x % 25; /* stockham.py:384 */
    lds[offset_lds + thread + 0]
        = {buf_inre[offset_in + (thread + 0) * stride0_in],
           buf_inim[offset_in + (thread + 0) * stride0_in]}; /* generator.py:872 */
    lds[offset_lds + thread + 25]
        = {buf_inre[offset_in + (thread + 25) * stride0_in],
           buf_inim[offset_in + (thread + 25) * stride0_in]}; /* generator.py:872 */
    lds[offset_lds + thread + 50]
        = {buf_inre[offset_in + (thread + 50) * stride0_in],
           buf_inim[offset_in + (thread + 50) * stride0_in]}; /* generator.py:872 */

    // append extra global loading for C2Real pre-process only
    if(ebtype == EmbeddedType::C2Real_PRE)
    {
        // use the last thread of each transform to load one more element per row
        if(thread == 24)
        {
            lds[offset_lds + thread + 50 + 1]
                = {buf_inre[offset_in + (thread + 50 + 1) * stride0_in],
                   buf_inim[offset_in + (thread + 50 + 1) * stride0_in]}; /* generator.py:872 */
        }
    }

    // handle even-length real to complex pre-process in lds before transform
    if(ebtype == EmbeddedType::C2Real_PRE)
    {
        __syncthreads();

        real_pre_process_kernel_inplace<scalar_type, false>(thread % 38 + 0,
                                                            75 - thread % 38 - 0,
                                                            38,
                                                            &lds[offset_lds],
                                                            0,
                                                            &twiddles[75]); /* stockham.py:338 */

        real_pre_process_kernel_inplace<scalar_type, false>(thread % 38 + 25,
                                                            75 - thread % 38 - 25,
                                                            38,
                                                            &lds[offset_lds],
                                                            0,
                                                            &twiddles[75]); /* stockham.py:338 */

        __syncthreads();
    }

    // transform
    write = true; /* stockham.py:681 */
    op_inverse_length75_SBRR_device<scalar_type, SB_UNIT>(
        lds, twiddles, stride_lds, offset_lds, write); /* stockham.py:686 */

    // handle even-length complex to real post-process in lds after transform
    if(ebtype == EmbeddedType::Real2C_POST)
    {
        __syncthreads();

        real_post_process_kernel_inplace<scalar_type, false>(thread % 38 + 0,
                                                             75 - thread % 38 - 0,
                                                             38,
                                                             &lds[offset_lds],
                                                             0,
                                                             &twiddles[75]); /* stockham.py:338 */

        real_post_process_kernel_inplace<scalar_type, false>(thread % 38 + 25,
                                                             75 - thread % 38 - 25,
                                                             38,
                                                             &lds[offset_lds],
                                                             0,
                                                             &twiddles[75]); /* stockham.py:338 */
    }

    // store global
    __syncthreads();
    buf_outre[offset_out + (thread + 0) * stride0_out]
        = lds[offset_lds + thread + 0].x; /* generator.py:906 */
    buf_outim[offset_out + (thread + 0) * stride0_out]
        = lds[offset_lds + thread + 0].y; /* generator.py:908 */
    buf_outre[offset_out + (thread + 25) * stride0_out]
        = lds[offset_lds + thread + 25].x; /* generator.py:906 */
    buf_outim[offset_out + (thread + 25) * stride0_out]
        = lds[offset_lds + thread + 25].y; /* generator.py:908 */
    buf_outre[offset_out + (thread + 50) * stride0_out]
        = lds[offset_lds + thread + 50].x; /* generator.py:906 */
    buf_outim[offset_out + (thread + 50) * stride0_out]
        = lds[offset_lds + thread + 50].y; /* generator.py:908 */

    // append extra global write for Real2C post-process only
    if(ebtype == EmbeddedType::Real2C_POST)
    {
        // use the last thread of each transform to write one more element per row
        if(thread == 24)
        {
            buf_outre[offset_out + (thread + 50 + 1) * stride0_out]
                = lds[offset_lds + thread + 50 + 1].x; /* generator.py:906 */
            buf_outim[offset_out + (thread + 50 + 1) * stride0_out]
                = lds[offset_lds + thread + 50 + 1].y; /* generator.py:908 */
        }
    }
}
POWX_SMALL_GENERATOR(rocfft_internal_dfn_dp_ci_ci_stoc_75,
                     ip_forward_length75_SBRR,
                     ip_inverse_length75_SBRR,
                     op_forward_length75_SBRR,
                     op_inverse_length75_SBRR,
                     double2);
POWX_SMALL_GENERATOR(rocfft_internal_dfn_sp_ci_ci_stoc_75,
                     ip_forward_length75_SBRR,
                     ip_inverse_length75_SBRR,
                     op_forward_length75_SBRR,
                     op_inverse_length75_SBRR,
                     float2);